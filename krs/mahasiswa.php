...
if ( $jenisusers == 2 )
{
    ...
    $online = false;
    //cek waktu krs per mahasiswa	
	$q = "SELECT a.TAHUN,a.SEMESTER,a.IDMAHASISWA,DATE_FORMAT(a.TANGGALMULAI,'%d-%m-%Y') AS TM,DATE_FORMAT(a.TANGGALSELESAI,'%d-%m-%Y') AS TS FROM ".
	"waktukrsmhs a JOIN mahasiswa b ON a.IDMAHASISWA=b.ID WHERE NOW() >= a.TANGGALMULAI AND NOW() <= a.TANGGALSELESAI AND ".
	"a.IDMAHASISWA='{$users}'";
	#echo $q.'<br>';
	$h = mysqli_query($koneksi,$q);
    if ( 0 < sqlnumrows( $h ) )
	...
	if ( $online != true )
    {
		//cek waktu krs per prodi per angkatan
		$q = "SELECT *,DATE_FORMAT(TANGGALMULAI,'%d-%m-%Y') AS TM,DATE_FORMAT(TANGGALSELESAI,'%d-%m-%Y') AS TS FROM waktukrsprodi WHERE ".
		"NOW() >= TANGGALMULAI AND NOW() <= TANGGALSELESAI AND PRODI='{$prodimahasiswa}' AND ANGKATAN='{$angkatanmahasiswa}'";
		#echo $q.'<br>';
		$h = mysqli_query($koneksi,$q);
		if ( 0 < sqlnumrows( $h ) )
		{
    ...
    {
		//cek waktu krs 
        $q = "SELECT *,DATE_FORMAT(TANGGALMULAI,'%d-%m-%Y') AS TM,DATE_FORMAT(TANGGALSELESAI,'%d-%m-%Y') AS TS FROM waktukrs WHERE ".
		"NOW() >= TANGGALMULAI AND NOW() <= TANGGALSELESAI";
		#echo $q;
		$h = mysqli_query($koneksi,$q);
        if ( 0 < sqlnumrows( $h ) )
        {
        ...
        if ( $statuskrsonline == 1 )
        {
            ...
            if ( $aksi == "updatemk" )
            {
                ...
                if ( is_array( $datax ) && $aksi2 == "ambil" && $UNIVERSITAS == "UNIVERSITAS BATAM" )
                {
					...
					if($dambilmk['ambilmk']>0){
						...
						echo "SKS BARU=".$sksbaru.'<br>';
						echo "JUMLAH SKS=".$jumlahsks.'<br>';
						echo "SKS WAJIB=".$jumlahskswajib.'<br>';
				...
				exit();
                        ...
                        {
                            $q = "DELETE FROM pengambilanmk WHERE IDMAHASISWA='{$idmahasiswa}' AND TAHUN='{$data['tahun']}' AND IDMAKUL='{$k}' AND SEMESTER='{$data['semester']}'";
                            mysqli_query($koneksi,$q);
                            if ( 0 < sqlaffectedrows( $koneksi ) )
                            {
                                $sksdiambil -= $v['sks'];
                                $ketlog = "Hapus Pengambilan Mata Kuliah dengan ID Makul={$k},Tahun Ajaran=".( $data[tahun] - 1 )."/{$data['tahun']},Semester=".$arraysemester[$data[semester]].",ID Mahasiswa={$idmahasiswa} ";
                                buatlog( 26 );
                                $q = "DELETE FROM trnlm WHERE NIMHSTRNLM='{$idmahasiswa}' AND THSMSTRNLM='".( $data[tahun] - 1 )."{$data['semester']}' AND KDKMKTRNLM='{$k}'";
                                mysqli_query($koneksi,$q);
                                $q = "SELECT IDMAHASISWA FROM pengambilanmk WHERE IDMAHASISWA='{$idmahasiswa}' AND TAHUN='{$data['tahun']}' AND SEMESTER='{$data['semester']}'";
                                $h = mysqli_query($koneksi,$q);
                                if ( sqlnumrows( $h ) <= 0 )
                                {
                                    $q = "delete FROM trakm WHERE NIMHSTRAKM='{$idmahasiswa}' AND THSMSTRAKM='".( $data[tahun] - 1 )."{$data['semester']}'";
                                    mysqli_query($koneksi,$q);
                                }
                                ++$jmlhapus;
                            }
            ...
            if ( $aksi == "tampiledit" && $halamankonfirmasi == 0 )
            {
				...
				if ( getaturan( "KRSONLINE" ) == 3 )
                {
					...
					$q = "SELECT BIAYA{$qf2} FROM biayakomponen WHERE ANGKATAN='{$angkatan}' AND IDPRODI='{$idprodi}' AND IDKOMPONEN='{$idkomponen}' AND GELOMBANG='{$gelombang}' {$qfieldjeniskelas} ";
				...
				#echo "STATUS KRS=".$statuskrs.'<br>';
				#exit();
                $tmpcetak = "";
                $idmahasiswa = $users;
                if ( $statuskrs == 0 && $biaya>0)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            echo getfield("STPIDMSMHS","msmhs","WHERE NIMHSMSMHS='$users' ");
			    if(getfield("STPIDMSMHS","msmhs","WHERE NIMHSMSMHS='$users' ")=="P"){
				$statusmahasiswa="Pindahan";
			    }else{
			    	$statusmahasiswa="Baru";
			    }
      ...
      tbkmk.THSMSTBKMK
		...
		";
											...
											<div class=\"m-section__content\">
												<div class=\"table-responsive\">
													<table class=\"table table-bordered table-hover\">
														<thead>
															<tr align=center class=juduldata>\n\t\t\t\t\t<td colspan=8 align=left>\n\t\t\t\t\t\t<a href='#' onClick='cekall();return false;'>[pilih semua]</a>  \n\t\t\t\t\t\t<a href='#' onClick='uncekall();return false;'>[batal pilih semua]</a> \n\t\t\t\t\t<input type=submit class=\"btn btn-brand\" name=aksi2 value='ambil' onClick=\"return confirm('Yakin Akan Diproses?');\">&nbsp;<input type=submit class=\"btn btn-brand\" name=aksi2 value='batal'  onClick=\"return confirm('Batalkan Pengambilan Mata Kuliah?');\"></td></tr>
						...
						$tmpcetak.="					</thead>
                                ...
                                while ( $dp = sqlfetcharray( $hp ) )
                                {
                                    ...
                                    if ( $semlama != $dp['SEMESTER'] )
                                    {
										...
										<input type=submit class=\"btn btn-brand\" name=aksi2 value='batal'  onClick=\"return confirm('Batalkan Pengambilan Mata Kuliah?');\">
														<script>
				    ...
				    $hss = mysqli_query($koneksi,$q);
								...
								$tmpcetak.="</tbody>";
                                $tmpcetak .= "<tr class=juduldata><td colspan=4>Total SKS Semester </td><td align=center>".$totalskskrs[$semlama]." </td>\n\t\t\t\t\t\t\t\t<td  >&nbsp;</td>\n\t\t\t\t\t\t\t\t<td  >&nbsp;</td>\n\t\t\t\t\t\t\t\t<td  >&nbsp;</td>\n\t\t\t\t\t\t\t</tr>";
								$tmpcetak .= "<tr class=juduldata>\n\t\t\t\t\t\t\t\t<td colspan=8 align=left>\n\t\t\t\t\t\t\t\t\t <a href='#' onClick='cekall{$semlama}();return false;'>[pilih semua] </a>\n\t\t\t\t\t\t\t\t\t<a href='#' onClick='uncekall{$semlama}();return false;'>[batal pilih semua]</a> \n\t\t\t\t\t<input type=submit class=\"btn btn-brand\" name=aksi2 value='ambil' onClick=\"return confirm('Proses Pengambilan Mata Kuliah Hanya Bisa Dilakukan Satu Kali,Yakin Akan Diproses?');\">&nbsp;<input type=submit class=\"btn btn-brand\" name=aksi2 value='batal'  onClick=\"return confirm('Batalkan Pengambilan Mata Kuliah?');\">
									<script>
										var start{$semlama}={$startawal};
										var count{$semlama}={$i};
										function cekall{$semlama}() {
                            ...
                            $q = "SELECT pengambilanmk.*,SKSMAKUL AS SKS,tbkmk.NAKMKTBKMK AS NAMA FROM pengambilanmk,tbkmk,msmhs WHERE ".
			    "pengambilanmk.IDMAHASISWA='{$idmahasiswa}'AND pengambilanmk.IDMAKUL=tbkmk.KDKMKTBKMK AND ".
			    "pengambilanmk.THNSM=tbkmk.THSMSTBKMK AND msmhs.KDPSTMSMHS=tbkmk.KDPSTTBKMK AND ".
			    "msmhs.KDJENMSMHS=tbkmk.KDJENTBKMK AND msmhs.NIMHSMSMHS=pengambilanmk.IDMAHASISWA AND ".
			    "tbkmk.KELOMPOKKURIKULUM='{$kelompokkurikulum}' {$qfield} ORDER BY pengambilanmk.TAHUN,pengambilanmk.SEMESTER";
                            #echo $q;
							$h = mysqli_query($koneksi,$q);
                            ...
                            else
                            {
								...
								$tmpcetak .= "<div align=center style='font-size:16pt;'>\n          <br> \n           <a href='cetakkrs.php?idmahasiswaupdate={$users}&tahunupdate={$data['tahun']}&semesterupdate={$data['semester']}'><b >Download KRS</b></a> </div>\n\t\t\t\t\t<br>\n\t\t\t\t";
