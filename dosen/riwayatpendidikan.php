...
if ( $aksi2 == "formupdate" )
{
    ...
    /*echo "\r\n  <br>\r\n\t\t<form name=form action=index.php method=post>\r\n\t\t<div class=\"portlet-body\">
						<div class=\"table-scrollable\">
                            <table class=\"table table-striped table-bordered table-hover\">".createinputhidden( "pilihan", $pilihan, "" ).createinputhidden( "aksi", "{$aksi}", "" ).createinputhidden( "sessid", $_SESSION['token'], "" ).createinputhidden( "idupdate", "{$idupdate}", "" ).createinputhidden( "tab", "{$tab}", "" ).createinputhidden( "urutan", "{$urutan}", "" )."\r\n \r\n  ";
    */
	echo createinputhidden( "pilihan", $pilihan, "" ).createinputhidden( "aksi", "{$aksi}", "" ).createinputhidden( "sessid", $_SESSION['token'], "" ).createinputhidden( "idupdate", "{$idupdate}", "" ).createinputhidden( "tab", "{$tab}", "" ).createinputhidden( "urutan", "{$urutan}", "" )."\r\n \r\n  ";
...
if ( $aksi2 == "formtambah" )
{
    ...
    */
	echo "<form name=form action=index.php method=post class=\"m-form m-form--fit m-form--label-align-right m-form--group-seperator\">".createinputhidden( "pilihan", $pilihan, "" ).createinputhidden( "sessid", $_SESSION['token'], "" ).createinputhidden( "aksi", "{$aksi}", "" ).createinputhidden( "tab", "{$tab}", "" ).createinputhidden( "idupdate", "{$idupdate}", "" )."\r\n \r\n  ";
	include( "formpendidikan.php" );
...
if ( $aksi2 == "" || $aksi2 == "tampilkan" )
{
    ...
    if ( 0 < sqlnumrows( $h ) )
    {
        /*echo "\r\n\t\t\t<br>\r\n\t\t\t\t<div class=\"portlet-body\">
						<div class=\"table-scrollable\">
                            <table class=\"table table-striped table-bordered table-hover\" {$border} class=form >\r\n \r\n\t\t\t\t\t<tr class=juduldata  align=center>\r\n\t\t\t\t\t\t<td>No</td>\r\n \t\t\t\t\t\t<td>Jenjang Studi</td>\r\n\t\t\t\t\t\t<td>Gelar</td>\r\n\t\t\t\t\t\t<td>Kode PT</td>\r\n\t\t\t\t\t\t<td>Nama PT</td>\r\n\t\t\t\t\t\t<td>Bidang Ilmu</td>\r\n\t\t\t\t\t\t<td>Kota Asal</td>\r\n\t\t\t\t\t\t<td>Kode Negara</td>\r\n\t\t\t\t\t\t<td>Tanggal<br>Ijazah</td>\r\n\t\t\t\t\t\t<td colspan=2>Aksi</td>\r\n\t\t\t\t\t</tr>";
        */
		echo "<div class=\"m-portlet\">			
        ...
        while ( $d = sqlfetcharray( $h ) )
        {
            ...
            $d2 = sqlfetcharray( $h2 );
            $q = "SELECT NMPTITBPTI FROM tbpti WHERE KDPTITBPTI='{$d['KODEPT']}'  ";
            $h3 = doquery($koneksi,$q);
            $d3 = sqlfetcharray( $h3 );
            if ( $d2[NMPTIMSPDS] == "" )
            {
                $d2[NMPTIMSPDS] = $d3[NMPTITBPTI];
            }
            $tmp = explode( "-", $d[TANGGALLULUS] );
            $tmp2[tgl] = $tmp[2];
            $tmp2[bln] = $tmp[1];
            $tmp2[thn] = $tmp[0];
            echo "\r\n\t\t\t\t\t<tr {$kelas} >\r\n\t\t\t\t\t\t<td>{$i} </td>\r\n \t\t\t\t\t\t<td align=center>".$arraypendidikantertinggi[$d2[JENJAMSPDS]]."</td>\r\n\t\t\t\t\t\t<td>{$d['GELAR']}</td>\r\n\t\t\t\t\t\t<td>{$d['KODEPT']}</td>\r\n\t\t\t\t\t\t<td>{$d2['NMPTIMSPDS']}</td>\r\n \r\n\t\t\t\t\t\t<td>{$d['BIDANG']}</td>\r\n\t\t\t\t\t\t<td align=center>{$d2['KOTAAMSPDS']}</td>\r\n\t\t\t\t\t\t<td align=center>{$d2['KDNEGMSPDS']}</td>\r\n\t\t\t\t\t\t<td align=center nowrap>{$tmp2['tgl']}-{$tmp2['bln']}-{$tmp2['thn']}</td>\r\n\t\t\t\t\t\t<td><a href='index.php?pilihan={$pilihan}&aksi={$aksi}&tab={$tab}&idupdate={$idupdate}&urutan={$d['ID']}&aksi2=formupdate'>".IKONUPDATE."</td>              \r\n\t\t\t\t\t\t<td><a onClick='return confirm(\"Hapus Data Riwayat Pendidikan Dosen?\")' href='index.php?pilihan={$pilihan}&aksi={$aksi}&tab={$tab}&idupdate={$idupdate}&urutan={$d['ID']}&aksi2=hapus&sessid={$_SESSION['token']}'>".IKONHAPUS."</td>              \r\n\t\t\t\t\t</tr>";
            ++$i;
        }
       # echo "\r\n\t\t\t\t</table></div></div>\r\n\t\t\t";
	   echo "								</tbody>
