...
$q = "SELECT COUNT(*) AS JML \r\n\tFROM dosenpengajarsp,makul,dosen,tbkmksp,mspst \r\n\tWHERE 1=1 {$qprodideptbkmksp}\r\n\tAND tbkmksp.KDKMKTBKMK=makul.ID\r\n\tAND tbkmksp.KDKMKTBKMK=dosenpengajarsp.IDMAKUL\r\n AND dosenpengajarsp.THSHM = tbkmksp.THSMSTBKMK\r\n\tAND dosen.ID=dosenpengajarsp.IDDOSEN \r\n\tAND dosenpengajarsp.IDPRODI=mspst.IDX\r\n\tAND mspst.KDPSTMSPST=tbkmksp.KDPSTTBKMK\r\n\tAND mspst.KDJENMSPST=tbkmksp.KDJENTBKMK\r\n\t{$qfield}\r\n \r\n\t";
$h = mysqli_query($koneksi,$q);
$d = sqlfetcharray( $h );
$total = $d[JML];
$first = 0;
include( "../paginating.php" );
$q = "SELECT tbkmksp.SEMESTBKMK AS  SEMESTERMAKUL,\r\n dosenpengajarsp.*,tbkmksp.NAKMKTBKMK AS NAMAMAKUL,dosen.NAMA AS NAMADOSEN \r\n\tFROM dosenpengajarsp,makul,dosen,tbkmksp,mspst \r\n\tWHERE 1=1 {$qprodideptbkmksp}\r\n\tAND tbkmksp.KDKMKTBKMK=makul.ID\r\n\tAND tbkmksp.KDKMKTBKMK=dosenpengajarsp.IDMAKUL\r\n AND dosenpengajarsp.THSHM = tbkmksp.THSMSTBKMK\r\n\tAND dosen.ID=dosenpengajarsp.IDDOSEN \r\n\tAND dosenpengajarsp.IDPRODI=mspst.IDX\r\n\tAND mspst.KDPSTMSPST=tbkmksp.KDPSTTBKMK\r\n\tAND mspst.KDJENMSPST=tbkmksp.KDJENTBKMK\r\n\t{$qfield}\r\n\tORDER BY ".$arraysort[$sort]." {$qlimit}";
$h = mysqli_query($koneksi,$q);
...
<div class=\"row\">
    ...
    echo "							{$tpage} {$tpage2}<div class=\"m-portlet\">			
										<div class=\"m-section__content\">
											<div class=\"table-responsive\">
												<table class=\"table table-bordered table-hover\">
													<thead>
														<tr class=juduldata{$aksi} align=center>\r\n\t\t\t\t<td>No</td>\r\n\t\t\t\t<td><a class='{$cetak}' href='{$href}"."&sort=0'>Prodi</td>\r\n\t\t\t\t<td><a class='{$cetak}' href='{$href}"."&sort=1'>Tahun Akademik </td>\r\n\t\t\t\t<td><a class='{$cetak}' href='{$href}"."&sort=2'>Se<br>mes<br>ter</td>\r\n\t\t\t\t<td><a class='{$cetak}' href='{$href}"."&sort=3'>Semes<br>ter<br>M-K</td>\r\n \t\t\t\t<td><a class='{$cetak}' href='{$href}"."&sort=4'>Kode Mata Kuliah</td>\r\n\t\t\t\t<td>Nama Mata Kuliah</td>\r\n \t\t\t\t<td><a class='{$cetak}' href='{$href}"."&sort=5'>NIP</td>\r\n \t\t\t<td>Dosen Pengajar</td>\r\n\t\t\t\t<td><a class='{$cetak}' href='{$href}"."&sort=6'>Kelas</td>";
	echo "													<td nowrap  >Cetak</td>
														</tr>";
	echo "												</thead>
														<tbody>";
    #echo "\r\n\t\t\t</tr>\r\n\t\t";
    ...
    while ( $d = sqlfetcharray( $h ) )
    {
        ...
        echo "\r\n\t\t\t\t\t\t\t\t<td   align=center>\r\n\t\t\t\t\t\t\t\t<a href='index.php?pilihan={$pilihan}&\r\n\t\t\t\t\t\t\t\taksi=formtambah&idprodiupdate={$d['IDPRODI']}&\r\n\t\t\t\t\t\t\t\tidmakulupdate={$d['IDMAKUL']}&\r\n\t\t\t\t\t\t\t\tiddosenupdate={$d['IDDOSEN']}&\r\n\t\t\t\t\t\t\t\ttahunupdate={$d['TAHUN']}&\r\n\t\t\t\t\t\t\t\tsemesterupdate={$d['SEMESTER']}&\r\n\t\t\t\t\t\t\t\tkelasupdate={$d['KELAS']}&jenis={$jenis}&datakuliah={$datakuliah}&kopsurat={$kopsurat}&jeniskelas={$jeniskelas}'>Cetak</td>\r\n \t\t\t\t\t\t\t\t";
        echo "\r\n\t\t\t\t</tr>\r\n\t\t\t";
        ++$i;
    }
