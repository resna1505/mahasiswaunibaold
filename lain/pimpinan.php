...
printjudulmenu( "Data Pimpinan dan Tenaga Non-Akademik" );
if ( $aksi == Hapus )
{
    if ( $_GET['sessid'] != $_SESSION['token'] )
    {
        $errmesg = token_err_mesg( "Pimpinan dan Tenaga Non-Akademik", HAPUS_DATA );
    }
    else
    {
        unset( $_SESSION['token'] );
        $q = "DELETE FROM trpim \r\n      WHERE KDPTITRPIM='{$kodept}' \r\n      AND THSMSTRPIM='{$tahun}{$semester}'\r\n      ";
...
if ( $aksi == "" )
{
    ...
    printjudulmenukecil( "<b>".IKONUPDATE." Edit Data Pimpinan dan Tenaga Non-Akademik" );
    ...
    $q = "SELECT THSMSTRPIM FROM trpim \r\n      WHERE KDPTITRPIM='{$kodept}' \r\n      ";
    $h = mysqli_query($koneksi,$q);
    if ( 0 < sqlnumrows( $h ) )
    {
    ...
    {
        printmesg( "Data Pimpinan dan Tenaga Non-Akademik tidak ada" );
    }
}
?>
