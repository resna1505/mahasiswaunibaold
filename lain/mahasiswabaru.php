...
if ( $aksi == "Hapus" )
{
    ...
    {
        $errmesg = token_err_mesg( "Kapasitas Mahasiswa Baru", HAPUS_DATA );
    }
    else
    {
        unset( $_SESSION['token'] );
        $q = "DELETE FROM trkap \r\n      WHERE KDPTITRKAP='{$kodept}' AND\r\n      KDPSTTRKAP='{$kodeps}' AND\r\n      KDJENTRKAP='{$kodejenjang}'\r\n      AND THSMSTRKAP='{$tahun}{$semester}'\r\n      ";
...
if ( $aksi == "" )
{
    ...
    printjudulmenukecil( "<b>".IKONUPDATE." Edit Data Kapasitas Mahasiswa Baru" );
    echo "\r\n <form action=index.php method=post>\r\n  <input type=hidden name=pilihan value='{$pilihan}'>\r\n  <table>\r\n    <tr>\r\n      <td>Jurusan/Program Studi</td>\r\n      <td>\r\n      <select name=idprodi>";
    $selected = "";
    foreach ( $arrayprodidep as $k => $v )
    {
    ...
    if ( $aksi2 == "Tampilkan" )
    {
        ...
        $q = "SELECT THSMSTRKAP FROM trkap \r\n      WHERE KDPTITRKAP='{$kodept}' AND\r\n      KDPSTTRKAP='{$kodeps}' AND\r\n      KDJENTRKAP='{$kodejenjang}'\r\n      ";
        $h = mysqli_query($koneksi,$q);
        if ( 0 < sqlnumrows( $h ) )
        {
        ...
        {
            printmesg( "Data Kapasitas Mahasiswa Baru untuk Program Studi ".$arrayprodidep[$idprodi]." tidak ada" );
        }
    }
}
?>
