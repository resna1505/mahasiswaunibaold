...
else
{
	...
	$q = "SELECT DATE_FORMAT(a.TANGGAL,'%d-%m-%Y') AS TANGGALTAGIHAN,b.ID,b.NAMA,c.ID AS IDKOMPONEN,a.BIAYA, DATE_FORMAT(a.TANGGALBAYAR2,'%d-%m-%Y') ".
	"AS BATASBAYAR FROM biayakomponen_tagihan a JOIN mahasiswa b ON a.IDPRODI=b.IDPRODI AND a.GELOMBANG=b.GELOMBANG AND a.ANGKATAN=b.ANGKATAN ".
	"JOIN komponenpembayaran c ON a.IDKOMPONEN=c.ID WHERE b.ID='{$users}' AND ".
	"YEAR(a.TANGGAL)>=YEAR(NOW()) ORDER BY a.TAHUN,a.SEMESTER,a.TANGGAL";
	echo $q.'<br>';
	#exit();
	...
	echo "							</thead>
		...
		while ( $d = sqlfetcharray( $h ) )
		{
			$waktu = "-";
				...
				echo "\r\n\t\t\t\t<tr align=center {$kelas}{$aksi} {$stylestrike}><td align=left nowrap>".$i."</td><td align=left nowrap>".$d['TANGGALTAGIHAN']."</td><td align=left nowrap>".$d['ID']."</td><td align=left>{$d['NAMA']}</td><td align=left>".$arraykomponenpembayaran[$d['IDKOMPONEN']]."</td><td align=right>".cetakuang($d['BIAYA'])."</td><td align=right>".$d['BATASBAYAR']."</td>";
