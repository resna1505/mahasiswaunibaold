	...
	if (count($_POST['pilih']) == 0) {
        ...
        SELECT x.NAMA,x.EMAIL2,x.HP,SUM(x.TOTAL + x.TOTALDENDA) as total FROM (
            ...
            SELECT
                ...
                a.BIAYA AS total
            FROM
                biayakomponen_tagihan_sks_mahasiswa AS a
                JOIN mahasiswa b ON a.IDMAHASISWA = b.ID
                LEFT JOIN biayakomponen_tagihan_mahasiswa as bta on bta.NOREC = a.NOREC
                JOIN komponenpembayaran c ON a.IDKOMPONEN = c.ID
                LEFT JOIN buattagihanva AS ba ON ba.NOREC = a.NOREC AND ba.IDMAHASISWA = b.ID
                JOIN biayakomponen AS bk ON bk.IDKOMPONEN = a.IDKOMPONEN 
                AND b.IDPRODI = bk.IDPRODI 
                AND b.ANGKATAN = bk.ANGKATAN 
                AND bk.IDPRODI = b.IDPRODI 
                AND bk.GELOMBANG = b.GELOMBANG
            WHERE
...
if ( $aksi == "tagihanvamahasiswa" )
{
	...
	x.NOREC,
	x.TOTALDENDA,
	( x.TOTAL + x.TOTALDENDA ) AS BIAYA 
FROM
	(
	SELECT
		CONCAT((a.TAHUN - 1), '/', a.TAHUN, CASE WHEN a.SEMESTER = 1 THEN ' Ganjil' WHEN a.SEMESTER = 2 THEN ' Genap' ELSE '-' END) AS TAHUN,
		a.SEMESTER,
		a.TANGGAL,
	...
	CASE
			...
			WHEN bk.JENISDENDA = 0 THEN
			( CASE WHEN bk.KATEGORIDENDA = 0 THEN ( bk.DENDA / 100 ) * a.BIAYA ELSE bk.DENDA END ) ELSE (
			CASE
					
					WHEN bk.KATEGORIDENDA = 0 THEN
					( bk.DENDA / 100 ) * (
					DATEDIFF( NOW(), a.TANGGALBAYAR2 )) ELSE bk.DENDA * DATEDIFF( NOW(), a.TANGGALBAYAR2 ) 
				END 
				) 
			END AS TOTALDENDA,
			db.DISKON,
		CASE
				...
				WHEN db.DISKON > 100 THEN
				bk.DENDA - db.DISKON ELSE ( 100 - ( CASE WHEN db.DISKON IS NULL THEN 0 ELSE db.DISKON END ) ) / 100 * bk.BIAYA 
			END AS TOTAL 
		FROM
			biayakomponen_tagihan a
			...
			AND bk.IDPRODI = b.IDPRODI 
			AND bk.GELOMBANG = b.GELOMBANG
			LEFT JOIN diskonbeasiswa AS db ON db.IDMAHASISWA = b.id 
			AND db.IDKOMPONEN = a.IDKOMPONEN 
			AND a.TAHUN = db.TAHUN 
		WHERE
			b.ID = '$users' 
			AND YEAR ( a.TANGGAL )>= YEAR (
			NOW()) 
			AND ba.STATUS IS NULL 
			AND bta.NOREC IS NULL UNION ALL
		SELECT
			CONCAT((a.TAHUN - 1), '/', a.TAHUN, CASE WHEN a.SEMESTER = 1 THEN ' Ganjil' WHEN a.SEMESTER = 2 THEN ' Genap' ELSE '-' END) AS TAHUN,
			a.SEMESTER,
			a.TANGGAL,
		...
		CASE
			...
			WHERE
				...
				NOW()) 
				AND ba.STATUS IS NULL 
				AND bta.NOREC IS NOT NULL UNION ALL
			SELECT
				CONCAT((a.TAHUN - 1), '/', a.TAHUN, CASE WHEN a.SEMESTER = 1 THEN ' Ganjil' WHEN a.SEMESTER = 2 THEN ' Genap' ELSE '-' END) AS TAHUN,
				a.SEMESTER,
				a.TANGGAL,
                                                    ...
                                                    echo '<div class="card p-3 m-4">
                                                        ...
                                                        <div class="row mt-2">
                                                            ...
                                                            <div class="col-6 col-sm-3 col-lg-3">
                                                                ...
                                                                <p style="font-size: 12px;">'.$listBelumBayar['TAHUN'].'</p>
                                                            </div>
                                                            <div class="col-6 col-sm-3 col-lg-3">
