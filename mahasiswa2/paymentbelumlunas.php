...
#echo $q;
$q="SELECT mahasiswa.IDPRODI,mahasiswa.NAMA,mahasiswa.ID,biayakomponen.IDKOMPONEN,komponenpembayaran.JENIS,biayakomponen.BIAYA AS TOTAL FROM mahasiswa,biayakomponen ,komponenpembayaran WHERE mahasiswa.ANGKATAN=biayakomponen.ANGKATAN AND mahasiswa.IDPRODI=biayakomponen.IDPRODI AND mahasiswa.GELOMBANG=biayakomponen.GELOMBANG AND biayakomponen.IDKOMPONEN=komponenpembayaran.ID AND komponenpembayaran.JENIS!=4 AND komponenpembayaran.JENIS!=6 AND komponenpembayaran.ID NOT IN ('103','120') AND biayakomponen.BIAYA>0 AND mahasiswa.ID = '{$idupdate}' AND biayakomponen.JENISKELAS='' ORDER BY mahasiswa.ID"; 
#echo $q.'<br>';
$h = doquery($koneksi,$q);
if ( 0 < sqlnumrows( $h ) )
{
 	//ambil tahun akademik terakhir krs mhs yg bersangkutan
	$q_krs_max = "SELECT MAX(pengambilanmk.TAHUN) AS max_thn_krs,MAX(pengambilanmk.SEMESTER) AS max_smstr_krs FROM pengambilanmk,tbkmk,msmhs ".
	"WHERE pengambilanmk.IDMAHASISWA='{$idmahasiswa}' ".
	...
	#echo $q_krs_max.'<br>';
	$h_krs_max = doquery($koneksi,$q_krs_max);
	$dt_krs_max=sqlfetcharray($h_krs_max);
	$tahunajaran=$dt_krs_max['max_thn_krs'];
	$semesterbayar=$dt_krs_max['max_smstr_krs'];   
	echo "<div class=\"m-portlet\">			
				<div class=\"m-section__content\">
					<div class=\"table-responsive\">
						<table class=\"table table-bordered table-hover\">
							<thead>";
    #echo "{$tpage} {$tpage2}\r\n \t\t\t<table {$border} class=data{$aksi}>\r\n\t\t\t<tr class=juduldata{$aksi} align=center>\r\n\t\t\t\t<td><b>No</td>\r\n \t\t\t\t<td><b>NIM</td>\r\n\t\t\t\t<td><b>Nama<br>Mahasiswa</td>\r\n\t\t\t\t<td><b>Komponen Pembayaran</td>\r\n\t\t\t\t<td><b>Jumlah Per Periode</td>\r\n  \t\t\t</tr>\r\n\t\t";
    echo "						<tr class=juduldata{$aksi} align=center><td><b>NIM</td>\r\n\t\t\t\t<td><b>Nama<br>Mahasiswa</td>\r\n\t\t\t\t<td><b>Komponen Pembayaran</td>\r\n\t\t\t\t<td><b>Jumlah Per Periode</td>\r\n  \t\t\t</tr>\r\n\t\t";
    echo "					</thead>
							<tbody>";
	$i = 1;
	...
	/*echo "TAHUN AJARAN AKADEMIK=".$tahunajaranakademik.'<br>';
	echo "SEMESTER AJARAN AKADEMIK=".$semesterajaranakademik.'<br>';
	echo "SEMESTER MHS SKRG=".$semestermhsskrg.'<br>';*/
	...
	$h_tot_krs = doquery($koneksi,$q_tot_krs);
		...
		while ($dt_tot_krs=sqlfetcharray($h_tot_krs)) {
			...
			elseif($d['IDKOMPONEN']!=99 && $d['IDKOMPONEN']!=98) { // NORMAL, BUKAN BIAYA TAMBAHAN SKS
					 ...
					 if ($d['TOTAL']>0 && $dt2['TOTAL']<$d['BATAS']) {
						if($idprodimahasiswa=='1003'){
							if($semestermhsskrg<8){
								...
								#echo "KOMPONENNYA=".$namakomponen.",BIAYA PER SEMESTER=".$d['TOTAL'].",TOTAL BIAYA PER SEMESTER=".$total_per_tahun_ajaran.'<br>';
				...
				if ($d['TOTAL']>0 && $dt2['TOTAL']<$d['BATAS']){ // Tidak Lunas
					...
					$total_tunggakan[$d['IDKOMPONEN']]['$idmahasiswa']=$total_biaya_sks_sp[$d['IDKOMPONEN']]['$idmahasiswa'];	
    ...
    }//end while looping pengambilan KRS data
    #echo "\r\n \t\t</table> ";
	echo "								</tbody>
								</table>
							</div>
