...
$q = "ALTER TABLE `sksmaksimum` ADD `JENISIP` SMALLINT NOT NULL ";
...
if ( $aksi == "Simpan" )
{
    ...
    else if ( is_array( $idupdate ) )
    {
        ...
        else
        {
            foreach ( $idupdate as $k => $v )
            {
                $q = "UPDATE syaratkrs SET SKS='".$sks[$k]."',IPS='".$ips[$k]."' \r\n      WHERE ID='{$k}' AND IDPRODI='{$idprodi}'";
                mysqli_query($koneksi,$q);
            }
...
if ( $aksi2 == "Simpan" )
{
    ...
    else
    {
        ...
        {
            $q = "INSERT INTO sksmaksimum (IDPRODI,SKS,SEMESTER,JENISIP) VALUES ('{$idprodi}','{$sksmaksimum}','{$semesteracuan}','{$jenisip}')";
            mysqli_query($koneksi,$q);
            if ( sqlaffectedrows( $koneksi ) < 0 )
            {
                $q = "UPDATE sksmaksimum SET SKS='{$sksmaksimum}',\r\n\t\t\tSEMESTER='{$semesteracuan}',\r\n\t\t\tJENISIP='{$jenisip}'\r\n\t\t\tWHERE IDPRODI='{$idprodi}'";
                mysqli_query($koneksi,$q);
            }
        }
    }
...
if ( $aksi == "Hapus" )
{
    ...
    else if ( is_array( $idupdate ) )
    {
        foreach ( $idupdate as $k => $v )
        {
            $q = "DELETE FROM syaratkrs WHERE ID='{$k}' AND IDPRODI='{$idprodi}'";
            mysqli_query($koneksi,$q);
        }
        if ( 0 < sqlaffectedrows( $koneksi ) )
        {
...
if ( $aksi == "Tambah" )
{
    ...
    else
    {
        ...
        {
            $idbaru = getnewidsyarat( "ID", "syaratkrs", " WHERE IDPRODI='{$idprodi}'" );
            $q = "INSERT INTO syaratkrs (ID,IDPRODI,SKS,IPS) VALUES('{$idbaru}','{$idprodi}','{$sksbaru}','{$ipsbaru}')";
            mysqli_query($koneksi,$q);
            if ( 0 < sqlaffectedrows( $koneksi ) )
            {
                    ...
                    <div class=\"portlet light\">
                        <div class=\"portlet-title\">
                            <div class=\"caption font-green-haze\">
                                ...
                                <span class=\"caption-subject bold uppercase\"> Syarat Pengambilan KRS </span>
                            </div>
                            <div class=\"actions\">
...
<div class=\"row\">
    ...
    $q = "SELECT * FROM syaratkrs WHERE IDPRODI='{$idprodi}' ORDER BY SKS DESC";
    ...
    $q = "SELECT * FROM sksmaksimum WHERE IDPRODI='{$idprodi}'";
    $h = mysqli_query($koneksi,$q);
    if ( 0 < sqlnumrows( $h ) )
    {
        $d = sqlfetcharray( $h );
