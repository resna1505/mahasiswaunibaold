...
if ( $aksi == "formupdate" )
{
    ...
    if ( 0 < sqlnumrows( $h ) )
    {
		...
		echo "			<div class='portlet-title'>";
							printmesg("Update Data Pengambilan M-K Mahasiswa");
		echo "		</div>";
...
if ( $aksi == "tampiledit" )
{
    ...
    else
    {
        ...
        else
        {
			...
			#echo "semester=".$data[semester].'<br>';exit();
...
<div class=\"row\">
                <div class=\"col-md-12\">
                    ...
                    $hip = doquery($koneksi,$q);
                    if ( 0 < sqlnumrows( $hip ) )
                    {
                        $dip = mysqli_fetch_array( $hip );
                        $ips = $dip[NLIPSTRAKM];
                    }
                    else
                    {
                    ...
                    */
                    ...
                    $hkrs = doquery($koneksi,$q);
                    if(0 < sqlnumrows( $hkrs )){
                        while ($dkrs = mysqli_fetch_array( $hkrs ))
                        {
					...
					//cek dulu klo dia mahasiswa beasiswa, di approve atau belum
					#echo "JUMLAH=".$d[JUMLAHBEASISWA];
                    ...
                    else
                    {
                        ...
                        */
                        ...
                        $semlama = "";
                        $startawal = 0;
                        while ( $dp = mysqli_fetch_array( $hp ) )
                        {
                            ...
                            if ( 0 < sqlnumrows( $hss ) )
                            {
								...
								while ( $dss = mysqli_fetch_array( $hss ) )
                                {
                                    $daftarsyarat .= "{$dss['IDSYARAT']}, {$dss['NILAI']}, {$dss['BOBOT']} <br>";
                                    $q = "SELECT pengambilanmk.NILAI,BOBOT,SIMBOL  \r\n            FROM pengambilanmk  \r\n            WHERE\r\n            pengambilanmk.IDMAKUL='{$dss['IDSYARAT']}' AND\r\n             IDMAHASISWA='{$idmahasiswa}'\r\n            AND BOBOT >= {$dss['BOBOT']}\r\n             ";
                                        $hss2 = doquery($koneksi,$q);
                            ...
                            if ( $syaratok == 1 )
                            {
                                ...
                                else if ( getaturan( "KELASKRSONLINE" ) == 1 )
                                {
                                    ...
                                    $hkelas = doquery($koneksi,$q);
                                    if ( 0 < sqlnumrows( $hkelas ) )
                                    {
                                        while ( $dkelas = mysqli_fetch_array( $hkelas ) )
                                        {
...
if ( $aksi == "tampilkan" )
{
    #echo "lll";exit();
    $aksi = " ";
    include( "prosestampilmahasiswakrs.php" );
}
if ( $aksi == "" )
{
...
<div class=\"row\">
    ...
    echo 	"	<div class=\"form-group m-form__group row\"  style=\"background-color:#b0c6f1;\">
				...
				<div class=\"form-group m-form__group row\" style=\"background-color:#b0c6f1;\">
					...
					<div class=\"col-lg-6\">
						".createinputtext( "makul", $makul, " class=form-control m-input  size=10 placeholder=\"Ketik Nama Mata Kuliah...\"" )."
