...
$vld[] = cekvaliditaskode( "Pengelompokan baris", $klpb, 2, false );
$vld[] = cekvaliditaskode( "Pengelompokan kolom", $klpk, 2, false );
$vld[] = cekvaliditaskode( "Grafik", $grafik, 1 );
$vld[] = cekvaliditaskodeprodi( "Jurusan/Program Studi", $idprodi );
$vld[] = cekvaliditastahun( "Angkatan", $angkatan );
$vld[] = cekvaliditaskode( "Status", $status, 2 );
$vld = array_filter( $vld, "filter_not_empty" );
if ( isset( $vld ) && 0 < count( $vld ) )
{
    $errmesg = "Data ".inv_message( $vld )." yang dipilih tidak valid";
    unset( $vld );
}
else
{
    ...
    if ( $status != "" )
    {
        ...
        $href .= "status={$status}&";*/
	if($thnakademik==1){
			$qfield .= " AND trlsm.THSMSTRLSM='{$tahun2}{$semester2}'";			
			$qjudul .= " Tahun Lulus '{$tahun2}{$semester2}' <br>";
			$qinput .= " <input type=hidden name=thnlulus value='{$tahun2}{$semester2}'>";
			$href .= "thnlulus={$tahun2}{$semester2}&";
		
		}
			#$qfield .= " AND STATUS='{$status}' AND trlsm.STMHSTRLSM='{$status}'";
			$qfield .= " AND STATUS='{$status}'";
			$qjudul .= " Status Mahasiswa '".$arraystatusmahasiswa["{$status}"]."' <br>";
			$qinput .= " <input type=hidden name=status value='{$status}'>";
			$href .= "status={$status}&";
    ...
    else if ( $klpb == 6 )
    {
		...
		if ( $thnakademik== 1 )
        {
			#$qfield .= " AND trlsm.STMHSTRLSM='{$status}'";
			$q = "SELECT DISTINCT THSMSTRLSM FROM trlsm WHERE trlsm.THSMSTRLSM='{$tahun2}{$semester2}'";
		}else{
			$q = "SELECT DISTINCT SUBSTR(THSMSTRLSM,1,4) AS THSMSTRLSM FROM trlsm ORDER BY THSMSTRLSM";
		}
		#echo $q.'<br>';
        $h = doquery($koneksi,$q);
        if ( 0 < sqlnumrows( $h ) )
        {
            while ( $d = sqlfetcharray( $h ) )
            {
                $arraygrupbaris[$d[THSMSTRLSM]] = $d[THSMSTRLSM];
            }
        }
        #$arraygrupbaris[$w[year]] = $w[year];
    }	
    ...
    else if ( $klpk == 6 )
    {
		...
		if ( $thnakademik== 1 )
        {
			...
			$q = "SELECT DISTINCT THSMSTRLSM AS THSMSTRLSM FROM trlsm ORDER BY THSMSTRLSM";
		}
		#echo "KLPK ENAM=". $q.'<br>';
        $h = doquery($koneksi,$q);
        if ( 0 < sqlnumrows( $h ) )
        {
            while ( $d = sqlfetcharray( $h ) )
            {
                $arraygrupkolom[$d[THSMSTRLSM]] = $d[THSMSTRLSM];
				#print_r($arraygrupkolom[$d[THSMSTRLSM]]).'<br>';
            }
        }
		#print_r($arraygrupkolom[$d[THSMSTRLSM]]);
		#print_r($arraygrupkolom).'<br>';
        #$arraygrupkolom[$w[year]] = $w[year];
    ...
    if ( is_array( $arraygrupbaris ) && is_array( $arraygrupkolom ) )
    {
			...
			echo "						<div class=\"tools\">
										<form target=_blank action='cetakrekap.php' method=post>
											...
											<table class=\"table table-striped2 table-bordered table-hover\">
												<tr>
													<td>
														<input type=submit name=aksi value=Cetak class=\"btn btn-brand\"></input>
		...
		if($klpb == 6 || $klpk == 6){
					...
					$q = "\r\n\t\t\t\t\t\tSELECT COUNT(DISTINCT(ID)) AS JML,".$arraygrup[$klpk]." AS XX FROM mahasiswa JOIN msmhs ON mahasiswa.ID=msmhs.NIMHSMSMHS LEFT JOIN trlsm ON trlsm.NIMHSTRLSM=mahasiswa.ID WHERE  ".$arraygrup[$klpb]."='{$kk}'\r\n\t\t\t\t\t\t{$qfield}\r\n\t\t\t\t\t\tGROUP BY ".$arraygrup[$klpk]."\r\n\t\t\t\t\t";
				   #echo 'KLPB I DAN KLPK II='.$q.'<br>';
					$h = doquery($koneksi,$q);
					unset( $data );
					while ( $d = sqlfetcharray( $h ) )
					{
