    ...
    if ( $_SESSION['token'] == $_POST['sessid'] )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if ( $sudahkirim == 0 )
                {
                    $q = "\r\n    \t\t\tINSERT INTO hasiltugaskuliahsp (IDBAHAN,IDMAKUL,TAHUN,KELAS,IDMAHASISWA,KET,SEMESTER,FILE,TANGGAL,FLAGFILE\r\n          ) \r\n    \t\t\tVALUES ('{$idupdate}','{$idmakulupdate}','{$tahunupdate}',\r\n    \t\t\t'{$kelasupdate}','{$users}','{$ket}','{$semesterupdate}','{$namafilebaru}',NOW(),1)\r\n    \t\t";
                }
                else if ( $sudahkirim == 1 && $d[KIRIMULANG] == 1 )
                {
                    $q = "\r\n    \t\t\tUPDATE hasiltugaskuliahsp SET \r\n          KET='{$ket}',\r\n          FILE='{$namafilebaru}',\r\n          TANGGAL=NOW()\r\n          WHERE\r\n            IDBAHAN='{$idupdate}' AND\r\n            IDMAKUL='{$idmakulupdate}' AND\r\n            TAHUN='{$tahunupdate}' AND\r\n            SEMESTER='{$semesterupdate}' AND\r\n            IDMAHASISWA='{$users}' AND\r\n            KELAS='{$kelasupdate}'";
                }
...
if ( $aksi == "" )
{
    ...
    $q = "SELECT tugaskuliahsp.*,IF(SELESAI<NOW(),1,0) AS TELAT \r\n    FROM tugaskuliahsp WHERE \r\n    IDBAHAN='{$idupdate}' AND\r\n    IDMAKUL='{$idmakulupdate}' AND\r\n    TAHUN='{$tahunupdate}' AND\r\n    SEMESTER='{$semesterupdate}' AND\r\n    KELAS='{$kelasupdate}'\r\n    ";
    $h = mysqli_query($koneksi,$q);
    $d = sqlfetcharray( $h );
    ...
    $pesanstatus = "<b style='color:red;'>Anda belum mengumpulkan tugas kuliah.</b>";
    if ( 0 < sqlnumrows( $hx ) )
    {
        $dx = sqlfetcharray( $hx );
        $nilai = $dx[NILAI];
        $statuskirim = 1;
        $pesanstatus = "<b style='color:blue;'>Anda sudah mengumpulkan tugas kuliah.</b>";
    }
    #echo "\r\n     \t\t <tr class=judulform>\r\n    \t\t\t<td class=judulform nowrap><b>Status</td>\r\n    \t\t\t<td>{$pesanstatus} </td>\r\n    \t\t</tr> ";
    echo "				<div class=\"form-group m-form__group row\">
    ...
    {
        #echo "\r\n     \t\t <tr class=judulform>\r\n    \t\t\t<td class=judulform nowrap><b>Nilai</td>\r\n    \t\t\t<td><b  style='font-size:24pt;'>{$nilai} </b> (Catatan: Nilai 0 bisa berarti dosen ybs belum mengupdate data nilai.)</td>\r\n    \t\t</tr> \r\n      ";
		echo "			<div class=\"form-group m-form__group row\" style=\"background-color:#f7f8fa;\">
    ...
    {
        #echo "\r\n\r\n \t\t <tr class=judulform>\r\n\t\t\t<td class=judulform nowrap><b> File Tugas Kuliah (Max {$d['UKURAN']}  MB) </td>\r\n\t\t\t<td><input type=file name=filebahan class=masukan size=40></td>\r\n\t\t</tr> \r\n \t\t <tr class=judulform>\r\n\t\t\t<td class=judulform nowrap><b> Catatan u/ Dosen</td>\r\n\t\t\t<td>".createinputtextarea( "ket", "{$ket}", "cols=50 rows=5" )."</td>\r\n\t\t</tr> \r\n\r\n\t\t\t<tr>\r\n\t\t\t\t<td colspan=2>\r\n\t\t\t\t<br>\r\n\t\t\t\t\t<input type=submit value='Upload File Ini' class=masukan>\r\n\t\t\t\t\t<input type=reset value='Reset' class=masukan>\r\n\t\t\t\t</td>\r\n\t\t\t</tr>";
		echo "			<div class=\"form-group m-form__group row\" style=\"background-color:#f7f8fa;\">
