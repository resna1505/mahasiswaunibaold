    ...
    if ( $_SESSION['token'] == $_POST['sessid'] )
    {
        ...
        else
        {
            ...
            else
            {
                if ( $filebahan_name == "" )
                {
                    $errmesg = "File Bahan kuliah harus diisi ";
                }
                else
                {
			
                    $idbaru = getnewidsyarat( "IDBAHAN", "bahankuliah", "\r\n\t\t\tWHERE IDMAKUL='{$idmakulupdate}' AND\r\n\t\t\tTAHUN='{$tahunupdate}' AND\r\n\t\t\tSEMESTER='{$semesterupdate}' AND\r\n\t\t\tKELAS='{$kelasupdate}'\r\n\t\t\t" );
                    
		    $namafilebaru = $filebahan_name;
		    #$namafilebaru = $filebahan;	

		    	#echo "FOLDER".$FOLDERFILE."ID BARU".$idbaru."FILE BARU".$namafilebaru.'<br>';
			...
			#echo "FILENYA".$namafilebaru.'<br>';

                    $ket = html_entity_decode( $ket );
                    $ket = preg_replace( "/<script[^>]*>.*?< *script[^>]*>/i", "", $ket );
			
                    /*if(move_uploaded_file($filebahan_name,"bahan/file/".md5( $namafilebaru ) )){
				echo "berhasil";
			}else{
			echo "tidak berhasil";
...
#exit();
			...
			$q = "\r\n\t\t\tINSERT INTO bahankuliah (IDBAHAN,IDMAKUL,TAHUN,KELAS,NAMA,KET,SEMESTER,FILE,TANGGAL,FLAGFILE,IDDOSEN) \r\n\t\t\tVALUES ('{$idbaru}','{$idmakulupdate}','{$tahunupdate}',\r\n\t\t\t'{$kelasupdate}','{$nama}','{$ket}','{$semesterupdate}','{$namafilebaru}',NOW(),1,'{$users}')\r\n\t\t";
                    mysqli_query($koneksi,$q);
                    if ( 0 < sqlaffectedrows( $koneksi ) )
                    {
						...
						#move_uploaded_file($filebahan_name,"{$FOLDERFILE}/".md5( $namafilebaru ) );
...
if ( $aksi == "formtambah" )
{
                ...
                <div class=\"col-md-12\">
                    ...
                    ";
                        ...
                        echo "			<div class='portlet-title'>";
												printmesg("Upload File Bahan Kuliah");
								echo "	</div>";
	...
	echo "				<div class='portlet-title'>";
							printmesg("Rincian Bahan Kuliah");
	echo "				</div>";
    $q = "\r\n\t\t\tSELECT IDBAHAN,NAMA,KET,FILE,TANGGAL,FLAGFILE,IDDOSEN FROM bahankuliah\r\n\t\t\tWHERE IDMAKUL='{$idmakulupdate}'\r\n\t\t\tAND TAHUN='{$tahunupdate}'\r\n\t\t\tAND SEMESTER='{$semesterupdate}'\r\n\t\t\tAND KELAS='{$kelasupdate}' \r\n\t\t\tORDER BY NAMA\r\n\t\t";
    ...
    if ( 0 < sqlnumrows( $h ) )
    {
        ...
        /*echo "			
							<form target=_blank action='cetakmakul2.php' method=post>"
								.createinputhidden( "pilihan", $pilihan, "" )
								.createinputhidden( "aksi", "formtambah", "" )
								.createinputhidden( "idmakulupdate", "{$idmakulupdate}", "" )
								.createinputhidden( "iddosenupdate", "{$iddosenupdate}", "" )
								.createinputhidden( "tahunupdate", "{$tahunupdate}", "" )
								.createinputhidden( "semesterupdate", "{$semesterupdate}", "" )
			...
			<div class=\"row\">
                <div class=\"col-md-12\">
                    ...
                    ";
						printmesg( $errmesg );
                        echo "			<div class='portlet-title'>";
												printmesg("Upload Bahan Mata Kuliah");
								echo "	</div>";
	echo "  	<div class=\"m-portlet\">				
					<!--begin::Form-->";
			...
			<div class=\"row\">
                <div class=\"col-md-12\">
                    ...
                    ";
						printmesg( $errmesg );
                        echo "			<div class='portlet-title'>";
												printmesg("Lihat File Bahan Kuliah");
								echo "	</div>";
						echo "
					<div class=\"m-portlet\">						
						<!--begin::Form-->
