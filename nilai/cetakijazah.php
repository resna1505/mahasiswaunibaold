...
if ( $aksi == "" )
{
    ...
    echo "	<div class=\"page-content\">
			...
			echo "						<div class='portlet-title'>";
											printtitle("Cetak Ijazah");
			$q = "SELECT *,DATE_FORMAT(TANGGALMULAI,'%d-%m-%Y') AS TM,\n      DATE_FORMAT(TANGGALSELESAI,'%d-%m-%Y') AS TS\n       FROM waktuijazah WHERE CURDATE() >= TANGGALMULAI AND\n      CURDATE() <= TANGGALSELESAI\n    ";
	#echo $q;
	$h = mysqli_query($koneksi,$q);
	if(sqlnumrows($h)==0){
		
		printmesg( "Waktu untuk Cetak Ijazah belum dibuka. Terima kasih" );
