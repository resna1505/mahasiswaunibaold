...
if ( $aksi == "tambah" )
{
    ...
    else
    {
        ...
        {
            $idbaru = getnewidsyarat( "IDKONVERSI", "konversiumum", "" );
            $q = "\r\n\t\t\t\tINSERT INTO konversiumum (IDKONVERSI,SIMBOL,NILAI,SYARAT) \r\n\t\t\t\tVALUES ('{$idbaru}','{$simbol}','{$data['nilai']}','{$data['syarat']}')\r\n\t\t\t";
            mysqli_query($koneksi,$q);
            if ( 0 < sqlaffectedrows( $koneksi ) )
            {
...
$_SESSION['token'] = $token;
 echo "<div class=\"page-content\">
		...
		echo "			<div class='portlet-title'>";
								printmesg("Edit Konversi Nilai Umum Default");
				echo "	</div>";
		...
		echo "			</div>	
						<div class=\"m-portlet\">
							...
							<!--begin::Form-->";
echo "					<form name=form class=\"m-form m-form--fit m-form--label-align-right m-form--group-seperator\" action=index.php method=post>".createinputhidden( "aksi", "tambah", "" ).createinputhidden( "sessid", "{$token}", "" ).createinputhidden( "tab", "{$tab}", "" ).createinputhidden( "pilihan", $pilihan, "" );
#printjudulmenukecil( "Data Konversi Nilai Baru" );
#echo "\r\n\t\t\t<table class=\"table table-striped table-bordered table-hover\">\r\n\t \t\t <tr class=judulform>\r\n\t\t\t\t<td class=judulform>Simbol</td>\r\n\t\t\t\t<td>".createinputtext( "simbol", "{$simbol}", "size=4 class=form-control m-input" )."</td>\r\n\t\t\t</tr>\r\n\t \t\t <tr class=judulform>\r\n\t\t\t\t<td class=judulform>Nilai Ekivalen (SKS)</td>\r\n\t\t\t\t<td>".createinputtext( "data[nilai]", "{$data['nilai']}", "size=4 class=form-control m-input" )."</td>\r\n\t\t\t</tr>"."<tr class=judulform>\r\n\t\t\t\t<td>Syarat</td>\r\n\t\t\t\t<td> Nilai >= ".createinputtext( "data[syarat]", "{$data['syarat']}", "size=4 class=form-control m-input" )." </td>\r\n\t\t\t\t</td>\r\n\t\t\t</tr>"."\r\n\t\t\t\t<tr>\r\n\t\t\t\t\t<td colspan=2>\r\n\t\t\t\t\t\t<input type=submit value='Tambah' class=\"btn blue\">\r\n\t\t\t\t\t\t<input type=reset value='Reset' class=\"btn default\">\r\n\t\t\t\t\t</td>\r\n\t\t\t\t</tr>\r\n\t\t\t\t</table>\r\n\t\t\t\t</form>\r\n\t\t\t<script>\r\n\t\t\t\tform.simbol.focus();\r\n\t\t\t</script>\r\n\t \t\t<br><br>";
echo "						
...
if ( 0 < sqlnumrows( $h ) )
{
    echo "\r\n\t\t\t<form name=form action=index.php method=post>".createinputhidden( "pilihan", $pilihan, "" ).createinputhidden( "tab", $tab, "" ).createinputhidden( "sessid", $token, "" )."
										...
										<div class=\"m-section__content\">
											<div class=\"table-responsive\">
												<table class=\"table table-bordered table-hover\">
													<thead>
														<tr class=juduldata align=center><td colspan=4></td><td><input type=submit name=aksitambah value='Update' class=\"btn btn-brand\"></td>\r\n\t\t\t\t\t\t<td ><input type=submit name=aksitambah value='Hapus' class=\"btn btn-secondary\"\r\n\t\t\t\t\t\tonClick=\"return confirm('Hapus Data Konversi?')\"\r\n\t\t\t\t\t\t></td>\r\n\t\t\t\t\t</tr>\r\n\t\t\t\t\t<tr class=juduldata align=center>\r\n\t\t\t\t\t\t<td>No</td>\r\n\t\t\t\t\t\t<td>Simbol</td>\r\n\t\t\t\t\t\t<td>Nilai</td>\r\n\t\t\t\t\t\t<td>Syarat</td>\r\n\t\t\t\t\t\t<td >Pilih Update</td>\r\n\t\t\t\t\t\t<td >Pilih Hapus</td>\r\n\t\t\t\t\t</tr>\r\n\t\t\t\t";
	echo "											</thead>
													<tbody>";
	$i = 1;
    $totalbobot = 0;
