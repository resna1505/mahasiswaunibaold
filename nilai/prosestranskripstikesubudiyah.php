...
if ( is_array( $arraydatatranskrip ) )
{
    ...
    if ( $semlama != "" )
    {
        ...
        $q = "SELECT SIMBOL FROM trlsm WHERE NIMHSTRLSM='{$d['ID']}' AND STMHSTRLSM='L' ORDER BY THSMSTRLSM";
        $hnilai = mysqli_query($koneksi,$q);
        $nilaiujiankomprehensif = "";
        if ( 0 < sqlnumrows( $hnilai ) )
        {
            $dnilai = sqlfetcharray( $hnilai );
            $nilaiujiankomprehensif = $dnilai[SIMBOL];
        }
        if ( $kompre == 1 )
        {
            $bodytranskrip .= "\n\t\t\t\t\t\t\t\t\t<tr><td>\n\t\t\t\t\t\t\t\tNilai Ujian Konfrehensif  </td><td>:  {$nilaiujiankomprehensif}<br>\n\t\t\t\t\t\t\t\t\t</td></tr>";
        }
        $bodytranskrip .= "\n                 \n                </table>";
        $bodytranskrip .= "\n\t\t\t\t\t\t\t\t</td>\n\t\t\t\t\t\t\t</tr>\n\t\t\t\t\t\t\t</table>\n\t\t\t\t\t\t";
