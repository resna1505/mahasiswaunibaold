...
{
    $online = false;
    $q = "SELECT ANGKATAN,IDPRODI FROM mahasiswa WHERE ID='{$users}'";
    ...
    if ( $online == true )
    {
        ...
        $q = "SELECT SUM(SKSMAKUL) TOTAL  FROM pengambilanmksp WHERE IDMAHASISWA='{$users}' AND TAHUN='{$tahunupdate}' \r\n AND SEMESTER='{$semesterupdate}'";
        $h = mysqli_query($koneksi,$q);
        $skssekarang = 0;
        if ( 0 < sqlnumrows( $h ) )
        {
    ...
    else
    {
        ...
        {
            printmesg( "Mahasiswa Baru (belum punya IP {$jtrakm}  Semester yg lalu) tidak dapat melakukan KRS Online. Terima kasih" );
        }
        else
        {
            if ( $aksi == "updatemk" )
            {
                if ( $aksi4 == "Ambil" )
                {
                    if ( $pilihkelas != "" )
                    {
                        ...
                        {
                            $q = "SELECT KELAS FROM jadwalkuliahkurikulumsp \r\n           WHERE\r\n          IDPRODI='{$idprodi}' AND\r\n          IDMAKUL='{$idmakul}' AND\r\n          KELAS='{$kelasmakul}' AND\r\n          JAM='{$jammulai}' AND\r\n          TAHUN='{$data['tahun']}' AND\r\n          SEMESTER='{$data['semester']}' AND\r\n          TERISI < KUOTA\r\n          ";
                            $h = mysqli_query($koneksi,$q);
                            if ( sqlnumrows( $h ) <= 0 )
                            {
                                $errmesg = "Maaf, kuota telah penuh. Proses pengambilan Mata Kuliah tidak berhasil dilakukan.";
                            }
                            else if ( $sksmaksimumsp < $sksmakul + $sksdiambil && $syaratkrssp == 1 && 0 <= $sksmaksimumsp )
                            {
                                $errmesg = "SKS yang hendak diambil {$k} (".( $v[sks] + $sksdiambil ).") melebihi batas maksimum SKS yang boleh diambil (".$sksmaksimumsp."). Proses pengambilan SKS Mata Kuliah dihentikan.";
                            }
                            else if ( $sksmaksimum2 < $sksmakul + $sksdiambil && 0 < $sksmaksimum2 )
                            {
                ...
                if ( $aksi4 == "Batal" )
                {
                    ...
                    if ( 0 < sqlaffectedrows( $koneksi ) )
                    {
                        ...
                        if ( $_SESSION['jeniskrs'] == 0 )
                        {
                            $q = "DELETE FROM pakaideposit WHERE\r\n            ID='SP-{$idmahasiswa}-{$idmakul}-{$data['tahun']}-{$data['semester']}'";
                            mysqli_query($koneksi,$q);
                        }
                    ...
                    else
                    {
                        ...
                        else
                        {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        if ( $syaratok == 1 )
                                        {
                                            ...
                                            $q = "SELECT jadwalkuliahkurikulumsp.* ,(KUOTA-TERISI) SISAKUOTA\r\n      from jadwalkuliahkurikulumsp \r\n      WHERE \r\n      IDMAKUL='{$idmakul}' AND \r\n      IDPRODI='{$idprodi}' AND \r\n      TAHUN='{$data['tahun']}' AND \r\n      SEMESTER='{$data['semester']}' \r\n      ORDER BY KELAS,JAM";
                            ...
                            else if ( $aksiproseskrs == "" )
                            {
                                ...
                                {
                                    $tmpcetak .= "\r\n      <!-- <td width=50%> -->\r\n\t\t<b><a id=0>Kurikulum Mata Kuliah Semester Ini</a>  :: [<a href='#1'>Mata Kuliah yang Telah Diambil</a>]</b><br>\r\n\t\t\t\t<table  class=form width=100%>\r\n \t\t\t\t<tr align=center class=juduldata>\r\n\t\t\t\t\t<td>No</td>\r\n\t\t\t\t\t<td>Kode</td>\r\n\t\t\t\t\t<td>Nama Mata Kuliah</td>\r\n\t\t\t\t\t<td>Wajib/Pilihan</td>\r\n\t\t\t\t\t<td>SKS</td>\r\n\t\t\t\t\t<td>Syarat</td>\r\n \t\t\t\t\t<td>Jumlah Jadwal</td>\r\n\t\t\t\t\t<td>Proses</td>\r\n\t\t\t\t</tr>\r\n\t\t\t";
                                    $i = 0;
                                    $semlama = "";
                                    $startawal = 0;
                                    while ( $dp = sqlfetcharray( $hp ) )
                                    {
                                        ...
                                        if ( $syaratok == 1 )
                                        {
                                            $jumlahjadwal = getfield( "COUNT(*)", "jadwalkuliahkurikulumsp", "WHERE IDMAKUL='{$dp['ID']}' AND IDPRODI='{$idprodi}' AND\r\n           TAHUN='{$data['tahun']}' AND SEMESTER='{$data['semester']}'" );
                                            $tmpcetak .= "\r\n \t\t\t\t\t\t<td>{$jumlahjadwal}</td>";
                                            if ( 0 < $jumlahjadwal )
                                            {
                            ...
                            $tmpcetak .= "\r\n\r\n      <!-- <td width=50%> -->\r\n      <br>\r\n       <b><a id=1>Mata Kuliah Yang Telah Diambil</a> :: [<a href='#0'> Kurikulum Mata Kuliah</a>]</b><br>\r\n\t\t\t";
                            $q = "SELECT IDMAHASISWA FROM pengambilanmksp \r\n          WHERE \r\n          IDMAHASISWA='{$idmahasiswa}'\r\n\t\t\t\t  AND TAHUN='{$data['tahun']}'\r\n          AND SEMESTER='{$data['semester']}'";
                            $h = mysqli_query($koneksi,$q);
                            if ( !( sqlnumrows( $h ) <= 0 ) || $pilihtampil2 == 0 )
                            {
                                $qfield = "";
                            }
                            else
            ...
            if ( $aksi == "" )
            {
                printjudulmenu( "Edit KRS Online" );
                printmesg( $errmesg );
                if ( $_SESSION[jeniskrs] == 1 )
                {
                    $strpkrs = "Perbaikan";
                ...
                echo "\r\n  <table>\r\n    <tr>\r\n      <td> \r\n    Anda dapat mengedit <b>{$strpkrs} Kartu Rencana Studi</b> untuk Semester Pendek ".$arraysemester[$semesterupdate]." ".( $tahunupdate - 1 )."/{$tahunupdate} dari tanggal <b>{$tanggalmulai}</b> s.d <b>{$tanggalselesai}</b>. Untuk mengedit KRS, silakan klik link di bawah ini\r\n    <br><br>\r\n    <a href='index.php?pilihan={$pilihan}&aksi=tampiledit&pilihtampil2=1'>Edit KRS</a>\r\n    </td>\r\n    </tr>\r\n    </table>\r\n    \r\n  ";
            }
        }
    }
}
