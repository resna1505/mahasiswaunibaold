...
$q = "SELECT * FROM jadwalkuliahsp WHERE IDMAKUL='{$idmakul}' AND TAHUN='{$tahun}' AND SEMESTER='{$semester}' AND IDPRODI='{$idprodi}' ORDER BY KELAS";
$h = mysqli_query($koneksi,$q);
if ( $from != "mobile" )
{
    printjudulmenu( "Mata Kuliah KL1041 - ".str_replace( "_", " ", $namamakul )." ".( $tahun - 1 )."/{$tahun} ".$arraysemester[$semester]."" );
}
else
{
    $tmpcetak .= "<h1>Mata Kuliah KL1041 - ".str_replace( "_", " ", $namamakul )." ".( $tahun - 1 )."/{$tahun} ".$arraysemester[$semester]."</h1>";
}
if ( 0 < sqlnumrows( $h ) )
{
    do
