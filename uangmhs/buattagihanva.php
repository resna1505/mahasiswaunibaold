...
if ( $aksi == "HAPUS" )
{
    ...
    {
        $q = "DELETE FROM buattagihanva WHERE TANGGALTAGIHAN='{$tanggal}'";
...
#$strunduh = "<form method=post action=downloadtagihanva.php target=_blank>\r\n<input type=hidden name=tanggal value=\"_TANGGAL_\">\r\n<table width=100% >\r\n<tr>\r\n  <td width=150 >\r\n    Tipe \r\n   </td>\r\n  <td   >\r\n    <input type=hidden name=jeniskolom value=\"{$jeniskolom}\" ><input type=hidden name=angkatancari value=\"{$angkatancari}\" ><input type=hidden name=idprodicari value=\"{$idprodicari}\" ><input type=hidden name=koderekening value=\"{$koderekening}\" > <input type=radio name=jenisfile value=\"CSV\" checked> CSV, delimiter <input type=text size=1 name=delimiter value=\";\">  </td>\r\n</tr>\r\n<tr>\r\n  <td  colspan=2>\r\n    <input type=submit name=aksi value=UNDUH>\r\n    <input type=submit name=aksi value=\"PERIKSA HASIL\">\r\n  </td>\r\n</tr>\r\n</table>\r\n</form>";
...
if ( $aksi == "Lanjut" )
{
	...
	if ( $koderekening != "" )
    {
        $qfield .= " AND komponenpembayaran.KODEREKENING='{$koderekening}' ";
    ...
    #if ( $tanggal != "" &&)
	{
        ...
        /*$q = "SELECT   komponenpembayaran.JENIS,c.TINGKAT,komponenpembayaran.LABELSPC,
		...
		{$qfield} ORDER BY   a.IDPRODI, a.ANGKATAN,\r\n      a.GELOMBANG {$qcolumn2} , a.IDKOMPONEN ";*/
		/*$q = "SELECT   komponenpembayaran.JENIS,c.TINGKAT,komponenpembayaran.LABELSPC,
		a.*,a.TANGGAL AS TANGGALTAGIH,DATE_FORMAT(a.TANGGAL,'%d-%m-%Y') AS TGLTAGIH,DATE_FORMAT(a.TANGGALBAYAR1,'%d-%m-%Y') AS TGLBAYAR1,
		DATE_FORMAT(a.TANGGALBAYAR2,'%d-%m-%Y') AS TGLBAYAR2 FROM komponenpembayaran,biayakomponen_tagihan a,
		(SELECT biayakomponen_tagihan.IDKOMPONEN, biayakomponen_tagihan.ANGKATAN, biayakomponen_tagihan.IDPRODI, 
		...
		{$qfield} 
		UNION ALL
		SELECT komponenpembayaran.JENIS,prodi.TINGKAT,komponenpembayaran.LABELSPC,trakk.IDKOMPONEN,mahasiswa.IDPRODI,mahasiswa.ANGKATAN,trakk.CCLTRAKK as BIAYA,
		...
		DATE_FORMAT(DATE_ADD(DATE_ADD(LAST_DAY(NOW()),INTERVAL 1 DAY),INTERVAL - 1 MONTH),'%d-%m-%Y') AS TGLTAGIH,
		DATE_FORMAT(DATE_ADD(DATE_ADD(LAST_DAY(NOW()),INTERVAL 1 DAY),INTERVAL - 1 MONTH),'%d-%m-%Y') AS TGLBAYAR1,
		DATE_FORMAT(DATE_ADD(DATE_ADD(LAST_DAY(NOW()),INTERVAL 20 DAY),INTERVAL - 1 MONTH),'%d-%m-%Y') AS TGLBAYAR2
		...
		JOIN prodi ON prodi.ID=mahasiswa.IDPRODI WHERE {$qfield2} ORDER BY 5,6,9,4";*/
        /*$q = "SELECT   komponenpembayaran.JENIS,komponenpembayaran.LABELSPC,a.*,a.TANGGAL AS TANGGALTAGIH,
		DATE_FORMAT(a.TANGGAL,'%d-%m-%Y') AS TGLTAGIH,DATE_FORMAT(a.TANGGALBAYAR1,'%d-%m-%Y') AS TGLBAYAR1,
		DATE_FORMAT(a.TANGGALBAYAR2,'%d-%m-%Y') AS TGLBAYAR2 FROM 
		komponenpembayaran,biayakomponen_tagihan a,
		(SELECT biayakomponen_tagihan.IDKOMPONEN, biayakomponen_tagihan.ANGKATAN, biayakomponen_tagihan.IDPRODI,
			biayakomponen_tagihan.GELOMBANG {$qcolumn2} ,MAX(IF(biayakomponen_tagihan.TANGGAL <= DATE_FORMAT('{$tanggal}' ,'%Y-%m-%d'),
			biayakomponen_tagihan.TANGGAL,'1901-01-01') )  AS TANGGALTAGIH 
		...
		WHERE komponenpembayaran.ID =a.IDKOMPONEN AND b.IDKOMPONEN=a.IDKOMPONEN AND b.IDPRODI=a.IDPRODI AND  b.ANGKATAN=a.ANGKATAN AND 
		b.GELOMBANG=a.GELOMBANG AND  {$qtagihan} b.TANGGALTAGIH=a.TANGGAL AND b.TANGGALTAGIH  <= DATE_FORMAT('{$tanggal}' ,'%Y-%m-%d')  
		{$qfield} ORDER BY   a.IDPRODI, a.ANGKATAN,a.GELOMBANG {$qcolumn2} , a.IDKOMPONEN ";*/
        $q = "SELECT   komponenpembayaran.JENIS,c.TINGKAT,komponenpembayaran.LABELSPC,komponenpembayaran.KODEBANK,komponenpembayaran.KODEREKENING,
		a.*,a.TANGGAL AS TANGGALTAGIH,DATE_FORMAT(a.TANGGAL,'%d-%m-%Y') AS TGLTAGIH,DATE_FORMAT(a.TANGGALBAYAR1,'%d-%m-%Y') AS TGLBAYAR1,
		DATE_FORMAT(a.TANGGALBAYAR2,'%d-%m-%Y') AS TGLBAYAR2 FROM komponenpembayaran,biayakomponen_tagihan a,
		(SELECT biayakomponen_tagihan.IDKOMPONEN, biayakomponen_tagihan.ANGKATAN, biayakomponen_tagihan.IDPRODI, 
		biayakomponen_tagihan.GELOMBANG {$qcolumn2} ,MAX(IF(biayakomponen_tagihan.TANGGAL <= DATE_FORMAT('{$tanggal}' ,'%Y-%m-%d'),
		...
		{$qfield} 
		UNION ALL
		SELECT   komponenpembayaran.JENIS,c.TINGKAT,komponenpembayaran.LABELSPC,komponenpembayaran.KODEBANK,komponenpembayaran.KODEREKENING,
...
$h = mysqli_query($koneksi,$q);
    ...
    if ( 0 < $jml )
    {
		...
		echo "			</div>";
        echo "			<form method=post action=downloadtagihanva.php target=_blank class=\"m-form m-form--fit m-form--label-align-right m-form--group-seperator\">
							<div class=\"m-portlet__body\">
								<div class=\"form-group m-form__group row\" style=\"background-color:#b0c6f1;\">
									<label class=\"col-lg-2 col-form-label\">Tanggal Tagihan</label>\r\n    
									<label class=\"col-form-label\">
