			...
			if ( is_array( $arraysettingcicilan ) )
			{
				foreach ( $arraysettingcicilan as $tanggalsetting => $datasetting )
				{
					...
					if($datasetting['TINGKAT']=='B'){
						...
						$qidkomponen="IDKOMPONEN IN (016,017,018,019,020,032,100,204,261)";
						...
						if($idkomponen=='012'){
							$qidkomponen="IDKOMPONEN IN (012,2014,211)";
						}else{
							$qidkomponen="IDKOMPONEN IN ({$idkomponen})";
						}
					...
					foreach ( $vv1 as $semestertagihan => $vv2 )
					{
						...
						if ( 0 < $v2[TAGIHAN] )
						{
							...
							if($idkomponen=="032"){
								...
								$totaltagihanpermahasiswa=$v2[TAGIHAN]+(($total_denda*$JUMLAHDENDA));
