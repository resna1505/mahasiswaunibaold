...
periksaroot( );
printjudulmenu( "Pembayaran Keuangan Rekening Lain Mahasiswa" );
if ( $aksi2 == "Simpan Data" && $REQUEST_METHOD == POST )
{
    if ( is_array( $arraypilih ) )
    {
        ...
        {
            $q = "UPDATE t_bni_ibank_lain2 SET description='".$arrayket[$k]."',USER='{$users}',TGLUPDATE=NOW() WHERE no_id='{$k}'";
            #echo $q;exit();
...
if ( $aksi == "Lanjut" )
{
    ...
    #{
        ...
        #{
            ...
            $q = "SELECT t_bni_ibank_lain2.*  FROM t_bni_ibank_lain2  WHERE  t_bni_ibank_lain2.no_id='{$no_id}'";
            #echo $q;
			$h = mysqli_query($koneksi,$q);
            ...
            if ( 0 < sqlnumrows( $h ) )
            {
                ...
                while ( $d = sqlfetcharray( $h ) )
                {
                    ...
                    echo "\r\n\t\t\t\t\t<tr  {$kelas} {$styletd}>\r\n\t\t\t\t\t\t<td  nowrap><input {$cektd}  type=checkbox name='arraypilih[{$d['no_id']}]' value=1></td>\r\n\t\t\t\t\t\t<td align=center nowrap>{$i}</td>\r\n \t\t\t\t\t\t<td  nowrap align=center>{$d['dates']}</td><td  nowrap align=right>".cetakuang($d['amount'])."</td></td>\r\n\t\t\t\t\t\t<td  nowrap width='30'><textarea cols=30 rows=2 name='arrayket[{$d['no_id']}]'>{$d['description']}</textarea></td></tr>";
