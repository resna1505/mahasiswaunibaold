...
if ( $aksi == "PROSES IMPOR" )
{
    ...
    else
    {
		...
		#echo '<br>';
		echo 'Data Bayar Mahasiswa=';
		echo '<br>';
		print_r($databayarmahasiswa);
		echo '<br>';
		#echo '<br>';
		#$naon=1;
		/*echo 'Data Amount=';
	...
	foreach ( $databayarmahasiswa as $k => $databayarbank )
    {
		...
		if(!empty($databayarmahasiswa[$k]['va_numb'])){
			...
			echo 'Data Bayar Bank=';
			print_r($databayarbank);
			echo '<br>';
				#echo $k;exit();
			echo "NIM=".$databayarmahasiswa[$k]['payee_id'].'<br>';

			echo "Data Bayar Mahasiswa=".$databayarmahasiswa[$k].'<br>';
			echo "KEY=".$k.'<br>';
			...
			$h = mysqli_query($koneksi,$q);
           	if (0 < sqlnumrows($h)) {
				...
				#$sql_get_bank="SELECT IDKOMPONEN AS idkomponen,TAHUN,SEMESTER,JUMLAH,DENDA,BEASISWA FROM buattagihanvamandiri WHERE 
				#VANUMB='{$k}' AND IDMAHASISWA='{$idmahasiswa}' AND TANGGALTAGIHAN='{$tanggal}' AND STATUS=0 ORDER BY TAHUN,SEMESTER";
				#$sql_get_bank="SELECT IDKOMPONEN AS idkomponen,TAHUN,SEMESTER,JUMLAH,DENDA,BEASISWA,TRXID FROM buattagihanvamandiri WHERE 
				#VANUMB='{$k}' AND IDMAHASISWA='{$idmahasiswa}' AND TANGGALTAGIHAN='{$tanggal}' AND STATUS!=1 ORDER BY TAHUN,SEMESTER";
				...
				$jmlDataTagihan=sqlnumrows($query_get_bank);
				if ($jmlDataTagihan>0) {
					while($dt=sqlfetcharray($query_get_bank)){
						...
						IDKOMPONEN='{$idkomponen}' AND IDPRODI='{$dm['IDPRODI']}' AND ANGKATAN='{$dm['ANGKATAN']}' ".
						"AND GELOMBANG='{$dm['GELOMBANG']}'";
						...
						#{
							...
							#while ($dt = sqlfetcharray( $ht )) {
								...
								#echo "IDKOMPONEN = ".$idkomponen.'<br>';
									...
									#echo "JUMLAH BIAYA=".$jumlahbiaya.'<br>';
									...
									$dibayar = 0;
								echo "JUMLAH BAYAR VA = ".$jmlbayarva.'<br>';	
								...
								$ketbyrmhs="IMPOR BY VA,TRX ID=".$dt['TRXID'];
									...
									{
										#if()
										echo "JUMLAH BAYAR VA LEBIH BESAR DARi TAGIHAN"."<br>";
										#$dibayar=$jumlah;
										#$sisa = $sisa - $dibayar;
									   ...
									   '{$carabayar}','0',CURDATE(),'{$users}',CURDATE(),'{$jumlahdendabayar}','".( $jumlahbiaya * 100 / ( 100 - $dt[BEASISWA] ) )."','{$dt['BEASISWA']}')";
										...
										echo "JUMLAH BAYAR VA LEBIH KECIL ATAU SAMA DENGAN TAGIHAN"."<br>"; 
									   ...
									   '{$carabayar}','0',CURDATE(),'{$users}',CURDATE(),'{$jumlahdendabayar}','".( $jumlahbiaya * 100 / ( 100 - $dt[BEASISWA] ) )."','{$dt['BEASISWA']}')";
										$dibayar=$jmlbayarva+$jumlahdendabayar;
									#}  
									echo $q.'<br>';
									#mysqli_query($koneksi,$q);
									#$jmlbayarva=$jmlbayarva-$jumlahtagihan;
									#echo "JUMLAH BAYAR VA DIKURANGI JML TAGIHAN=".$jmlbayarva.'<br>';
    ...
    else
    {
        ...
        */
		...
		else if ( $arrayjeniskomponenpembayaran[$dt_cek_total_tagihan['IDKOMPONEN']] == 5 )
		{
			...
			$htbayar = mysqli_query($koneksi,$qbayar);
			$dtbayar = sqlfetcharray( $htbayar );
			$bayartotalmhs=$sudahbayar+$bayarkurangdenda;
	    ...
	    if($dtCekBayar['recordbayar']>0 && $NIM!='61120043'){
				...
				$hasil .= "<tr {$kelas}><td nowrap>&nbsp;</td><td nowrap>{$VANUMB} </td><td nowrap><input type='text' name='databayarmahasiswa[$i][payee_id]' value='{$PAYEE_ID}'>{$NIM} </td><td nowrap>{$NAME} </td><td><input type=text size=8 name='databayarmahasiswa[$i][payment_date]' value='{$PAYMENTDATE}' readonly></td> <!-- <td>{$ADDRESS_1}</td>\r\n      <td>{$BILL_REF_1}</td>\r\n      <td>{$BILL_REF_2}</td>\r\n      <td>{$BILL_REF_3}</td>\r\n      <td>{$BILL_REF_4}</td>\r\n      <td>{$BILL_REF_5}</td> --><td><input type=text size=8 name='databayarmahasiswa[$i][payment_amount]' value='{$PAYMENTAMOUNT}' readonly><!--\r\n      <td>{$AUTODEBET_ACC_D}</td>\r\n      <td>{$REGISTER_NO}</td>\r\n      <td>{$DUE_DATE}</td>\r\n    -->\r\n    </tr>    \r\n    \r\n    ";
            }
	    elseif($bayartotalmhs>$totalcicilanmhs){
				$txtpay='';
				$kelas = "style='background-color:#ffaaaa'";
				$hasil .= "<tr {$kelas}><td nowrap><input type=checkbox name='databayarmahasiswa[$i][va_numb]'   value='{$VANUMB}' {$txtpay}></td><td nowrap>{$VANUMB} </td><td nowrap><input type='text' name='databayarmahasiswa[$i][payee_id]' value='{$PAYEE_ID}'>{$NIM} </td><td nowrap>{$NAME} </td><td><input type=text size=8 name='databayarmahasiswa[$i][payment_date]' value='{$PAYMENTDATE}' readonly></td> <!-- <td>{$ADDRESS_1}</td>\r\n      <td>{$BILL_REF_1}</td>\r\n      <td>{$BILL_REF_2}</td>\r\n      <td>{$BILL_REF_3}</td>\r\n      <td>{$BILL_REF_4}</td>\r\n      <td>{$BILL_REF_5}</td> --><td><input type=text size=8 name='databayarmahasiswa[$i][payment_amount]' value='{$PAYMENTAMOUNT}' readonly><!--\r\n      <td>{$AUTODEBET_ACC_D}</td>\r\n      <td>{$REGISTER_NO}</td>\r\n      <td>{$DUE_DATE}</td>\r\n    -->\r\n    </tr>    \r\n    \r\n    ";
   ...
   # echo $q;
    ...
    if ( 0 < $jml )
    {
		...
		echo "			<div class='portlet-title'>";
								printtitle("Impor Pembayaran VA Bank Mandiri");
		echo "			</div>";
