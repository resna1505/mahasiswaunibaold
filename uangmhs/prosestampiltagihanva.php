...
else
{
    ...
    /*$q = "SELECT COUNT(DISTINCT(mahasiswa.ID)) AS JML \r\n  FROM msmhs {$qtabel}  ,mahasiswa LEFT JOIN mahasiswa2 ON mahasiswa.ID=mahasiswa2.ID LEFT JOIN trlsm ON trlsm.NIMHSTRLSM=mahasiswa.ID\r\n\tWHERE \r\n  mahasiswa.ID=msmhs.NIMHSMSMHS\r\n   {$qprodidep2}\r\n\t{$qfield}\r\n\t";
    #echo $q.'<br>';
    ...
    $q = "SELECT komponenpembayaran.LABELSPC,komponenpembayaran.JENIS,buattagihanva.*,buattagihanva.JUMLAH AS JUMLAH,buattagihanva.STATUS AS SUDAHDIPROSES,
	buattagihanva.STATUS AS TOTALDATA,a.NAMA,a.IDPRODI,a.ANGKATAN ,a.ALAMAT,prodi.NAMA AS NAMAPRODI, prodi.TINGKAT,
	departemen.NAMA AS NAMAJURUSAN,fakultas.NAMA AS NAMAFAKULTAS FROM mahasiswa a, prodi, departemen LEFT JOIN fakultas 
	ON fakultas.ID=departemen.IDFAKULTAS,buattagihanva,komponenpembayaran WHERE departemen.ID=prodi.IDDEPARTEMEN AND 
	a.IDPRODI=prodi.ID AND buattagihanva.IDKOMPONEN=komponenpembayaran.ID AND 
	a.ID=buattagihanva.IDMAHASISWA AND buattagihanva.TANGGALTAGIHAN='{$tanggal}' AND komponenpembayaran.LABELSPC!='' {$qfield} 
    ...
    if ( 0 < sqlnumrows( $h ) )
    {
        ...
        while ( $d = sqlfetcharray( $h ) )
        {
            ...
            echo "\r\n\t\t\t\t<tr align=center {$kelas}{$aksi}><td> <input type='checkbox' class='chkbox' name='ids[]' value='".$i."' ></td><td align=left nowrap>".$d[TANGGALTAGIHAN]."</td><td align=left nowrap>".$d[IDMAHASISWA]."</td><td align=left>{$d['NAMA']}</td><td align=left>".$d[ANGKATAN]."</td><td align=left>".$d[NAMAPRODI]."</td><td align=left>{$d['IDKOMPONEN']}/".$arraykomponenpembayaran[$d[IDKOMPONEN]]."</td><td align=left>{$waktu}</td><td align=left><input type='text' name='jml_tagihan_[{$i}]' value='{$d[JUMLAH]}'><td align=left><input type='text' size='10' maxlength='10' name='expired_tagihan_[{$i}]' value='{$d[EXPDATE]}'><input type='hidden' name='tgl_tagihan_[{$i}]' value='{$d[TANGGALTAGIHAN]}'><input type='hidden' name='komponen_tagihan_[{$i}]' value='{$d[IDKOMPONEN]}'><input type='hidden' name='id_mhs_[{$i}]' value='{$d[IDMAHASISWA]}'><input type='hidden' name='semester_ajaran_[{$i}]' value='{$d[SEMESTER]}'><input type='hidden' name='tahun_ajaran_[{$i}]' value='{$d[TAHUN]}'></td></td>";
							...
							<table class=\"table table-striped table-bordered table-hover\">
								<tr>
									<td width=\"1%;\">
										...
										<input type=submit name=aksi value='simpandata' class=\"btn btn-brand\">
										<input type=submit name=aksi2 value='hapusdata' class=\"btn btn-brand\">
