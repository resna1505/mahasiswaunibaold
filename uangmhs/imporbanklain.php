...
printjudulmenu( "IMPOR PEMBAYARAN BANK CALON MAHASISWA - {$tanggal}" );
        ...
        foreach ( $datamahasiswa as $k => $idmahasiswa )
        {
            ...
            if (0 < sqlnumrows( $h )) {
				...
				while ($idspc <= 10) {
                        ...
                        if ( 0 < $jumlah )
                        {
                            $q = "SELECT * FROM buattagihan_calonmahasiswa WHERE IDMAHASISWA='{$idmahasiswa}' AND IDKOMPONEN='{$idkomponen}' AND TANGGALTAGIHAN='{$tanggal}' AND STATUS=0 ORDER BY TAHUN,SEMESTER ";
...
if ( $aksi == "LANJUT" )
{
    ...
    else
    {
        ...
        foreach ( $data as $k => $v )
        {
            ...
            if ( $angkatan != "" && $angkatan != getfield( "TAHUN", "calonmahasiswa", "WHERE ID='{$PAYEE_ID}'" ) )
            {
                continue;
            }
            if ( $idprodi != "" && $idprodi != getfield( "PRODI1", "calonmahasiswa", "WHERE ID='{$PAYEE_ID}'" ) )
            {
                continue;
            }
