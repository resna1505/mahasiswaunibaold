...
if ( $aksi == "hapus" )
{
    ...
    {
        $q = "DELETE FROM buattagihanva WHERE TANGGALTAGIHAN='{$tanggal}' AND IDMAHASISWA='{$idmhs}' AND IDKOMPONEN='{$idkomponen}'
...
if ( $aksi == "simpandata" )
{
    ...
    else
    {
		...
		foreach($ids as $id){		
			...
			$query = "UPDATE buattagihanva SET JUMLAH = '{$jml_tagihan}',EXPDATE='{$expired_tagihan}' WHERE IDKOMPONEN = '{$komponen_tagihan}' AND TANGGALTAGIHAN='{$tgl_tagihan}'
			...
			/*exit();
			#$tgl_tagihan = '';
			foreach($tgl_tagihan_array as $tgl_tagihan) {
...
if ( $aksi2 == "hapusdata" )
{
    ...
    else
    {
		...
		foreach($ids as $id){		
			...
			$semester_ajaran = $_POST['semester_ajaran_'][$id];
			$tahun_ajaran = $_POST['tahun_ajaran_'][$id];
			$query = "DELETE FROM buattagihanva WHERE IDKOMPONEN = '{$komponen_tagihan}' AND TANGGALTAGIHAN='{$tgl_tagihan}'
						AND IDMAHASISWA='{$id_mhs}' AND TAHUN='{$tahun_ajaran}' AND SEMESTER='{$semester_ajaran}'";
							#echo $query.'<br>';
							mysqli_query($koneksi,$query);
			if ( 0 < sqlaffectedrows( $koneksi ) )
...
if ( $aksi == "tampilkan" )
{
	#echo "aaa";exit();
    $aksi = " ";
    include( "prosestampiltagihanva.php" );
}
if ( $aksi == "" )
{
    #printmesg( $errmesg );
...
<div class=\"row\">
    ...
    if ( 0 < $jml )
    {
		...
		echo "			<div class='portlet-title'>";
								printtitle("Koreksi Hasil Pembuatan Tagihan VA Sebelumnya");
		echo "			</div>";
