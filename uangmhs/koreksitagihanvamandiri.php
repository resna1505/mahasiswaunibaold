...
if ( $aksi == "hapus" )
{
    ...
    {
        $q = "DELETE FROM buattagihanvamandiri WHERE TANGGALTAGIHAN='{$tanggal}' AND IDMAHASISWA='{$idmhs}' AND IDKOMPONEN='{$idkomponen}'
...
if ( $aksi == "simpandata" )
{
    ...
    else
    {
		...
		foreach($ids as $id){		
			...
			$jml_denda = $_POST['jml_denda_'][$id];
			$expired_tagihan = $_POST['expired_tagihan_'][$id];
			$semester_ajaran = $_POST['semester_ajaran_'][$id];
			$tahun_ajaran = $_POST['tahun_ajaran_'][$id];
			$va_numb = $_POST['va_numb_'][$id];
			$trx_id_lama = $_POST['trx_id_lama_'][$id];
			$va_numb_lama = $_POST['va_numb_lama_'][$id];
			$exp_date_lama = $_POST['exp_date_lama_'][$id];
			$query = "UPDATE buattagihanvamandiri SET DENDA = '{$jml_denda}',JUMLAH = '{$jml_tagihan}',EXPDATE='{$expired_tagihan}',
						VANUMB='{$va_numb}',IDUSER='{$users}',TGLUPDATE=NOW() WHERE IDKOMPONEN = '{$komponen_tagihan}' AND TANGGALTAGIHAN='{$tgl_tagihan}'
						AND IDMAHASISWA='{$id_mhs}' AND TAHUN='{$tahun_ajaran}' AND SEMESTER='{$semester_ajaran}' 
						AND VANUMB='{$va_numb_lama}' AND EXPDATE='{$exp_date_lama}'";
							#echo $query.'<br>';
							mysqli_query($koneksi,$query);
			if ( 0 < mysqli_affected_rows( $koneksi ) )
			{
	...
	#$nim = "(".$tmp.")";
	$tanggal=$tgl_tagihan;
	$angkatancari=$_POST['angkatan_mhs'];
	$idprodicari=$_POST['prodi_mhs'];
...
if ( $aksi2 == "hapusdata" )
{
    ...
    else
    {
		...
		foreach($ids as $id){		
			...
			$tahun_ajaran = $_POST['tahun_ajaran_'][$id];
			$va_numb = $_POST['va_numb_'][$id];
			$query = "DELETE FROM buattagihanvamandiri WHERE IDKOMPONEN = '{$komponen_tagihan}' AND TANGGALTAGIHAN='{$tgl_tagihan}'
						AND IDMAHASISWA='{$id_mhs}' AND TAHUN='{$tahun_ajaran}' AND SEMESTER='{$semester_ajaran}' AND VANUMB='{$va_numb}'";
							#echo $query.'<br>';
							mysqli_query($koneksi,$query);
			if ( 0 < mysqli_affected_rows( $koneksi ) )
			{
...
if ( $aksi == "tampilkan" )
{
	#echo "aaa";exit();
    $aksi = "";
    include( "prosestampiltagihanvamandiri.php" );
}
if ( $aksi == "" )
{
    #printmesg( $errmesg );
...
<div class=\"row\">
    ...
    if ( 0 < $jml )
    {
		...
		echo "			<div class='portlet-title'>";
								printmesg("Koreksi Tagihan VA Mandiri");
		echo "			</div>";
    ...
    echo "								</select>
								...
								<div class=\"form-group m-form__group row\">
									...
									<label class=\"col-form-label\">
										<select class=form-control m-input name=idprodicari><!--<option value=''>Semua</option>-->";
