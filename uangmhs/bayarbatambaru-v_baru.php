    ...
    else
    {
		...
		if ( $aksi2 == "Lanjut" && getaturan( "KEUANGAN2" ) == 1){
			...
			if($arrayjeniskomponenpembayaran[$idkomponen] == 3 && $idkomponen != 99 && $idkomponen != 98 && $idkomponen!=265){
				...
				#echo "TAHUN AWAL=".$tahunawal.'<br>';
				#echo "TAHUN LALU=".$tahunlalu.'<br>';
				if ( $tahunawal == $tahun && $semester <= $semesterawal )
				{
				}
				else if ( $tahunawal <= $tahunlalu )
				{
					#echo "AAA".$tahunmulai.$semestermulai.'<br>';
					#echo "CCC".$tahun.$semester.'<br>';
					#exit();		
					#echo "Record=".$data_schedule['record'];
       ...
       echo "<div class=\"page-content\">
        ...
        if ( $aksi2 == "" )
        {
          ...
          <script>
            ...
            echo "</script><select name=idkomponen  onChange='gantilabel(this.value);' class=form-control m-input >  <option value='' >Pilih Komponen Pembayaran</option>\r\n           ";
            foreach ( $arraykomponenpembayaran_baru as $k => $v )
            {
           ...
           # echo "\r\n              </select><tr id=pertahun style='display:none;'>\r\n          <td>Tahun Ajaran</td>\r\n           <td>\r\n            \r\n\t\t\t\t\t\t<select name=tahunajaran class=form-control m-input> \r\n\t\t\t\t\t\t ";
												...
												$arrayangkatan = getarrayangkatan( "B" );
            ...
            echo "							</select>
									...
									<div id=persemester class=\"form-group m-form__group row\" style=\"background-color:#f7f8fa;display:none;\">
										...
										<label class=\"col-form-label\">
											".createinputtahunajaransemesterbayar( $semua = 0, $tahun = "tahun", $semester = "semester", $addnumber = 0, $plus1 = 0, $tambah1 = 1 )."
										</label>
									</div>
        ...
        if ( $aksi2 == "Lanjut" )
        {
			...
			$biaya = $totalbayar = $totaldiskon = $ketdiskon = $diskon = $dibayar = $ketsks = $denda = "";
					...
					#print_r($arraykomponenpembayaran);
					$qtahunsemester2 = "";
					...
					$q = "SELECT biayakomponen.BIAYA,IF(biayakomponen.DENDA='0',$JUMLAHDENDA,biayakomponen.DENDA) AS DENDA,IF(biayakomponen.TANGGAL='0',20,biayakomponen.TANGGAL) AS TANGGAL,'1',biayakomponen_tagihan.BIAYA as cicilan 
					FROM biayakomponen,komponenpembayaran,biayakomponen_tagihan WHERE biayakomponen.IDKOMPONEN=komponenpembayaran.ID 
					AND biayakomponen_tagihan.IDKOMPONEN= biayakomponen.IDKOMPONEN AND biayakomponen_tagihan.ANGKATAN= biayakomponen.ANGKATAN 
					AND biayakomponen_tagihan.IDPRODI= biayakomponen.IDPRODI AND biayakomponen_tagihan.GELOMBANG= biayakomponen.GELOMBANG 
					AND biayakomponen.IDKOMPONEN='{$idkomponen}' 
					...
					AND trakk_tagihan.IDMAHASISWA=trakk.NIMHSTRAKK WHERE trakk.NIMHSTRAKK='$idmahasiswa' AND trakk.IDKOMPONEN='{$idkomponen}' {$qtambah}";
            ...
            if ( 0 < sqlnumrows( $h2 ) )
            {
                $d2 = sqlfetcharray( $h2 );
                $databiayakomponen = $d2;
				if($data_schedule['record']>0 && $tahun.$semester < $tahunmulai.$semestermulai && ($idkomponen=="032" || $idkomponen=="007")){					
					#echo "BIAYA=0";
					$biaya = 0;
				}else{
					#echo "BIAYA=NILAI";
			...
			#echo "JENIS KOMPONEN".$arrayjeniskomponenpembayaran[$idkomponen];exit();
  ...
  if (  v=='') { // Tidak memilih
    ...
    document.getElementById('cuti').style.display='';
   	...
   	";  
        ...
        #$q = "SELECT bayarkomponen.*,biayakomponen.BIAYA AS BIAYA2,\r\n\t\t\t IF(DATE(bayarkomponen.TANGGAL)=CURDATE(),1,0) AS STATUSTANGGAL,\r\n\t\t\t DATE_FORMAT(TANGGALBAYAR,'%d-%m-%Y') AS TGLBAYAR\r\n       FROM bayarkomponen,biayakomponen ,mahasiswa\r\n       WHERE \r\n       mahasiswa.ID=bayarkomponen.IDMAHASISWA AND\r\n       mahasiswa.ANGKATAN=biayakomponen.ANGKATAN AND\r\n       mahasiswa.IDPRODI=biayakomponen.IDPRODI AND\r\n       mahasiswa.GELOMBANG=biayakomponen.GELOMBANG AND\r\n       bayarkomponen.IDKOMPONEN=biayakomponen.IDKOMPONEN AND\r\n        \r\n      IDMAHASISWA='{$idmahasiswa}'      ORDER BY bayarkomponen.JENIS,bayarkomponen.IDKOMPONEN,bayarkomponen.TAHUNAJARAN,bayarkomponen.SEMESTER,TANGGALBAYAR";
        if($data_schedule['record']>0){
			$q = "SELECT bayarkomponen.*,biayakomponen.BIAYA AS BIAYA2,\r\n\t\t\t IF(DATE(bayarkomponen.TANGGAL)=CURDATE(),1,0) AS STATUSTANGGAL,
            ...
            */
            ...
            while ( $d = sqlfetcharray( $h ) )
            {
                ...
                )
				{
                  //$sisa=$d[BIAYA];
                  $sisa=$biaya;
				  #echo "IDKOMPONEN=".$d[IDKOMPONEN]." SISA=".$sisa.'<br>';
