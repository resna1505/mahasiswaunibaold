...
if ( $jenisusers == 0 )
{
	...
	biayakomponen_tagihan.IDKOMPONEN='{$idkomponen}' AND 
	biayakomponen_tagihan.IDPRODI='{$idprodi}' AND 
	biayakomponen_tagihan.ANGKATAN='{$angkatan}'  
	AND biayakomponen_tagihan.TANGGAL='{$tgl}' 
	UNION ALL
	SELECT trakk_tagihan.IDKOMPONEN,mahasiswa.IDPRODI,trakk_tagihan.ANGKATAN,trakk_tagihan.BIAYA,trakk_tagihan.TANGGAL,trakk_tagihan.GELOMBANG,
	trakk_tagihan.JENISKELAS,trakk_tagihan.TAHUN,trakk_tagihan.SEMESTER,trakk_tagihan.TANGGALBAYAR1,trakk_tagihan.TANGGALBAYAR2,
	trakk_tagihan.TANGGAL AS TANGGALTAGIH FROM trakk_tagihan JOIN mahasiswa ON mahasiswa.ID=trakk_tagihan.IDMAHASISWA WHERE 
	trakk_tagihan.IDKOMPONEN='{$idkomponen}' AND 
	mahasiswa.IDPRODI='{$idprodi}' AND 
	trakk_tagihan.ANGKATAN='{$angkatan}'  
	AND trakk_tagihan.TANGGAL='{$tgl}' ";*/
	...
	biayakomponen_tagihan.JENISKELAS=biayakomponen.JENISKELAS AND  prodi.ID=biayakomponen.IDPRODI AND 
	biayakomponen_tagihan.IDKOMPONEN='{$idkomponen}' AND 
	biayakomponen_tagihan.IDPRODI='{$idprodi}' AND biayakomponen_tagihan.ANGKATAN='{$angkatan}'  {$qfieldbiaya} AND biayakomponen_tagihan.TANGGAL <= '{$tanggal}'  {$qtahunsemester} 
	UNION ALL
	SELECT trakk_tagihan.*,trakk.JMLTRAKK AS BIAYAASLI,prodi.TINGKAT,trakk.DENDA,trakk.JENISDENDA,trakk.TGLBTSTRAKK AS TGL_AKHIR,trakk.KATEGORIDENDA
	FROM trakk_tagihan,trakk,prodi,mahasiswa WHERE 
	trakk_tagihan.IDKOMPONEN=trakk.IDKOMPONEN AND  mahasiswa.IDPRODI=prodi.ID AND  
	...
	trakk_tagihan.JENISKELAS=mahasiswa.JENISKELAS AND trakk_tagihan.IDMAHASISWA=mahasiswa.ID AND
	trakk_tagihan.IDKOMPONEN='{$idkomponen}' AND trakk.NIMHSTRAKK=trakk_tagihan.IDMAHASISWA AND
	prodi.ID='{$idprodi}' AND trakk_tagihan.ANGKATAN='{$angkatan}'  {$qfieldbiaya} AND trakk_tagihan.TANGGAL <= '{$tanggal}' {$qtahunsemester} ";*/
	echo "TAGIHAN BNI=".$q.'<br>';
	$h = mysqli_query($koneksi,$q);
    if(0 < sqlnumrows( $h )){
	...
	$q = "SELECT mahasiswa.ID,mahasiswa.NAMA  FROM mahasiswa WHERE mahasiswa.IDPRODI='{$idprodi}' 
	AND mahasiswa.ANGKATAN='{$angkatan}'  {$qfieldmahasiswa} {$qstatusmahasiswa} ORDER BY mahasiswa.ID  ";	
   echo "TAGIHAN BNI=".$q.'<br>';
	$h = mysqli_query($koneksi,$q);
    if(0 < sqlnumrows( $h )){		
		$i=0;
			...
			if ( is_array( $arraysettingcicilan ) )
			{
				foreach ( $arraysettingcicilan as $tanggalsetting => $datasetting )
				{
						...
						echo "TOTAL TAGIHAN=".$totaltagihan["".$datasetting[TAHUN]]["".$datasetting[SEMESTER]].'<br>';
							...
							if(strtotime($tanggal)>strtotime($datasetting['TANGGALBAYAR2'])){
									...
									if($datasetting['KATEGORIDENDA']==0){
										...
										$totaldenda["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]=$totaldenda["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]+$denda;
									}									
									
							}else{
								echo "TANGGAL TAGIHAN LEBIH KECIL".'<br>';
								$totaldenda["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]=$totaldenda["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]+0;
					...
					if($d2['TOTAL']!=NULL || $d2['TOTAL']!=''){ //jika ada pembayaran dalam rentang waktu dalam setting cicilan
						...
						$totalsudahbayar["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]=$totalsudahbayar["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]+$d2['TOTAL'];
						$totaltagihan["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]=$totaltagihan["".$datasetting['TAHUN']]["".$datasetting['SEMESTER']]+$datasetting['BIAYA'];
				  	}	
				...
				foreach ( $totalsudahbayar as $tahuntagihan => $vv1 )
				{
					foreach ( $vv1 as $semestertagihan => $vv2 )
					{
						...
						{							
							...
							#$VANUMB=$kodebank2.$koderekening2.$idkomponen.substr(number_format(time() * rand(),0,'',''),0,4);
							//cek dulu VA NUMB dengan id komponen, tanggal tagihan dan kode rekening ada yang sama ga
							$sql_data_va="SELECT COUNT(VANUMB) AS jml_data_va,VANUMB AS nomor_va FROM buattagihanvabni WHERE IDKOMPONEN='{$idkomponen}' ".
							"AND TANGGALTAGIHAN='{$tanggal}' AND SUBSTRING(VANUMB,4,5)='{$koderekening2}' AND IDMAHASISWA='{$idmahasiswa}'";
							echo "CEK DATA VA=".$cek_data_va.'<br>';
							$query_data_va=doquery($koneksi,$sql_data_va);
							$jml_data_va=sqlfetcharray($query_data_va);
							if($jml_data_va['jml_data_va']>0){
								print_r($jml_data_va);
								echo '<br>';
								$VANUMB=$jml_data_va['nomor_va'];
								echo "NUMB=".$VANUMB.'<br>';
							
							}else{
								if(strlen($idkomponen)=='3'){
									$VANUMB=$kodebank2.$koderekening2.$idkomponen.substr(number_format(time() * rand(),0,'',''),0,5);
								}else{
									$VANUMB=$kodebank2.$koderekening2.$idkomponen.substr(number_format(time() * rand(),0,'',''),0,4);
								}
								#$VANUMB=$kodebank2.$koderekening2.$idkomponen.substr(number_format(time() * rand(),0,'',''),0,4);
							...
							$q = "INSERT IGNORE INTO buattagihanvabni ( IDMAHASISWA,IDKOMPONEN,JUMLAH,DENDA,TANGGAL,TANGGALTAGIHAN,JENISKOLOM,TAHUN,SEMESTER,BEASISWA,TRXID,VANUMB,EXPDATE,EXPTIME,IDUSER) VALUES 
							( '{$idmahasiswa}','{$idkomponen}','{$v2['TAGIHAN']}','{$total_denda_akhir}',NOW(),'{$tanggal}','{$jeniskolom}','{$tahuntagihan}','{$semestertagihan}','{$beasiswa}','{$TRXID}','{$VANUMB}','{$exp_date_tagihan}','23:59:59','{$users}')";
