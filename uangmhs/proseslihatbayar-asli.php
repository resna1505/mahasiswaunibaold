...
if ( 0 < sqlnumrows( $h ) )
{
    ...
    while ( $d = sqlfetcharray( $h ) )
    {
        ...
        if ( $tingkataksesusers[$kodemenu] == "T" )
        {
            ...
            {
                echo "\r\n  \t\t\t\t\t\t\t\t<td   align=center><a href='index.php?pilihan=bayar&aksi=Lanjut&idmahasiswa={$d['IDMAHASISWA']}&tglbayar[tgl]={$tglbayar['tgl']}&tglbayar[bln]={$tglbayar['bln']}&tglbayar[thn]={$tglbayar['thn']}&carabayar={$d['CARABAYAR']}&aksilanjut=edit&count=1&jeniskomponen_0={$d['IDKOMPONEN']}'>".IKONUPDATE."</td>\r\n  \t\t\t\t\t\t\t\t<td   align=center ><a \r\n  \t\t\t\t\t\t\t\tonclick=\"return confirm('Hapus Data  Pembayaran? ');\"\r\n  \t\t\t\t\t\t\t\thref='{$href}&aksi2=hapus&idmahasiswahapus={$d['IDMAHASISWA']}&tanggalhapus={$d['TANGGALBAYAR']}&idkomponenhapus={$d['IDKOMPONEN']}'>".IKONHAPUS."</td>\r\n  \t\t\t\t\t\t\t\t";
            }
            else
            {
                echo "\r\n                  <td>-</td>\r\n                  <td>-</td>\r\n                ";
