...
if ( $aksi == "hapus" )
{
    ...
    {
        $q = "DELETE FROM buattagihanvabni WHERE TANGGALTAGIHAN='{$tanggal}' AND IDMAHASISWA='{$idmhs}' AND IDKOMPONEN='{$idkomponen}'
...
if ( $aksi == "Update" )
{
    ...
    else
    {
		...
		foreach($ids as $id){		
			...
			$query = "UPDATE buattagihanvabni SET DENDA = '{$jml_denda}',JUMLAH = '{$jml_tagihan}',EXPDATE='{$expired_tagihan}',
	...
	$koderekening2=$_POST['kode_rekening2'];
    $aksi = "tampilkan";
}
if ( $aksi2 == "Hapus" )
{
    ...
    else
    {
		...
		foreach($ids as $id){		
			...
			$tahun_ajaran = $_POST['tahun_ajaran_'][$id];
			$va_numb = $_POST['va_numb_'][$id];
			$query = "DELETE FROM buattagihanvabni WHERE IDKOMPONEN = '{$komponen_tagihan}' AND TANGGALTAGIHAN='{$tgl_tagihan}'
...
if ( $aksi == "tampilkan" )
{
	#echo "aaa";exit();
    $aksi = " ";
    include( "prosestampilkoreksitagihanvabni.php" );
}
if ( $aksi == "" )
{
    #printmesg( $errmesg );
...
<div class=\"row\">
    ...
    if ( 0 < $jml )
    {
		...
		echo "			<div class='portlet-title'>";
								printmesg("Koreksi Tagihan VA BNI");
		echo "			</div>";
